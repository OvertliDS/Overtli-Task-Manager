...
Then reinstall the latest version globally.`;
  ...
  assert.deepEqual(
    tasks.find((task) => task.title === "Fix prompt route segmentation")
      .internalSteps,
    [
...
3. Diagnostics repair flow`);
  assert.deepEqual(
    planning
      .filter((task) => /^Plan /.test(task.title))
      .map((task) => task.title),
    [
      "Plan Runtime install lane",
      "Plan Model manager UX",
      "Plan Diagnostics repair flow",
    ],
  );
  assert.equal(
    planning.some(
  ...
  assert.equal(
    docs.at(-1).title,
    "Reconcile evidence and prepare final summary",
  );
