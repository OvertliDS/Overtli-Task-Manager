...
test("MCP results provide Markdown and a redacted structured envelope", () => {
  ...
  const fallback = toMcpResult({
    stopAllowed: false,
    remainingRequired: [{ title: "Finish tests" }],
  });
  assert.match(fallback.content[0].text, /audit blocked/i);
  assert.doesNotMatch(fallback.content[0].text, /remainingRequired/);
  assert.deepEqual(fallback.structuredContent.result.remainingRequired, [
    { title: "Finish tests" },
  ]);
});
...
test("MCP schemas close input/output objects and reject nested unsafe arguments", () => {
  ...
  for (const tool of mcpTools) {
    ...
    for (const [field, schema] of Object.entries(
      ...
      if (schema.type === "string")
        assert.ok(
          schema.maxLength,
          `${tool.name}.${field} must declare maxLength`,
        );
      if (schema.type === "array")
        assert.ok(
          schema.maxItems,
          `${tool.name}.${field} must declare maxItems`,
        );
    }
  }
  assert.equal(
  ...
  assert.equal(
    mcpTools.find((tool) => tool.name === "otm_abandon").annotations
      .destructiveHint,
    true,
  );
  assert.throws(() => validateMcpArgs("otm_snapshot", { unknown: true }), {
    code: "MCP_UNKNOWN_ARGUMENT",
  });
  assert.throws(
    () => validateMcpArgs("otm_start", { goal: "x".repeat(16_001) }),
    { code: "INPUT_TOO_LARGE" },
  );
  const cyclic = {};
  cyclic.context = cyclic;
  assert.throws(
    () => validateMcpArgs("otm_start", { goal: "valid", context: cyclic }),
    { code: "CYCLIC_CONTEXT" },
  );
  assert.throws(
    () =>
      validateMcpArgs("otm_start", {
        goal: "valid",
        tasks: [{ title: "nested", unexpected: true }],
      }),
    { code: "MCP_UNKNOWN_ARGUMENT" },
  );
  assert.throws(
    () =>
      validateMcpArgs("otm_complete_task", {
        taskId: "task",
        evidence: { summary: "x", leaked: true },
      }),
    { code: "MCP_UNKNOWN_ARGUMENT" },
  );
  assert.throws(
    () =>
      validateMcpArgs("otm_reconcile", {
        changes: Array.from({ length: 257 }, () => ({
          action: "add",
          title: "task",
        })),
      }),
  ...
  assert.throws(
    () => validateMcpArgs("otm_cleanup_workspace", { minAgeMs: -1 }),
    { code: "MCP_INVALID_ARGUMENTS" },
  );
  assert.throws(
    () =>
      validateMcpArgs("otm_progress", {
        message: "ok",
        internalStepIndex: 1.5,
      }),
    { code: "MCP_INVALID_ARGUMENTS" },
  );
  assert.throws(
    () =>
      validateMcpArgs("otm_complete_task", {
        taskId: "task",
        evidence: { summary: "ok", exitCode: 256 },
      }),
    { code: "MCP_INVALID_ARGUMENTS" },
  );
  assert.throws(() => validateMcpArgs("otm_abandon", { runId: "run" }), {
    code: "MCP_INVALID_ARGUMENTS",
  });
});
...
test("MCP resource URI scope is explicit and safe", () => {
  assert.deepEqual(
    parseScopedResourceUri(
      "otm://workspace/C%3A%5CWork%5CProject/session/thread-42/current.md",
    ),
    { workspace: "C:\\Work\\Project", sessionId: "thread-42", markdown: true },
  );
  assert.equal(parseScopedResourceUri("otm://current"), null);
  assert.throws(
    () => parseScopedResourceUri("otm://workspace/%E0%A4/session/x/current"),
...
test("MCP stdio protocol rejects malformed arguments and returns scoped structured results", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-mcp-protocol-state-"),
  );
  ...
  try {
    ...
    assert.equal(
      listed.tools.find((tool) => tool.name === "otm_archive").annotations
        .destructiveHint,
      true,
    );
    ...
    assert.equal(
      fs.readdirSync(stateDir).length,
      0,
