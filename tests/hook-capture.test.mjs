...
async function evidenceFor(mode, command) {
  ...
  const env = {
    ...process.env,
    OTM_STORAGE: "json",
    OTM_STATE_DIR: fs.mkdtempSync(
      path.join(os.tmpdir(), "otm-hook-capture-state-"),
    ),
    CODEX_THREAD_ID: "capture-session",
    OTM_COMMAND_CAPTURE: mode,
  };
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  const started = manager.start({
    workspaceRoot,
    goal: "Capture policy",
    tasks: [{ title: "Inspect capture" }],
  });
  ...
  return (
    manager.store.getTask(started.snapshot.tasks[0].id).evidence.at(-1) || null
  );
}
...
test("hook command capture policy supports none, redacted, and validation-only modes", async () => {
  ...
  const validationOnlyNonValidation = await evidenceFor(
    "validation-only",
    "echo secret-value",
  );
  assert.equal(validationOnlyNonValidation, null);
  const validationOnlyTest = await evidenceFor(
    "validation-only",
    "npm test -- --runInBand",
  );
  assert.ok(validationOnlyTest);
  assert.match(validationOnlyTest.command, /npm test/);
  const redacted = await evidenceFor(
    "redacted",
...
test("hook command capture honors the injected environment over process-global state", async () => {
  ...
  try {
    const evidence = await evidenceFor(
      "redacted",
      "npm test -- token=injected-secret",
    );
...
test("distinct tool events in one turn are not deduplicated when the host omits a tool-use id", async () => {
  ...
  const env = {
    ...
    OTM_STORAGE: "json",
    OTM_STATE_DIR: fs.mkdtempSync(
      path.join(os.tmpdir(), "otm-hook-dedupe-state-"),
    ),
    CODEX_THREAD_ID: "tool-events-session",
  };
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  const started = manager.start({
    workspaceRoot,
    goal: "Record distinct validation events",
  ...
  for (const command of ["npm test -- first", "npm test -- second"]) {
    await runHookScript("post-tool-use", {
      ...
      stdin: JSON.stringify({
        cwd: workspaceRoot,
        session_id: "tool-events-session",
        turn_id: "shared-turn",
        tool_name: "Bash",
        tool_input: { command },
        tool_response: { exit_code: 0 },
  ...
  assert.deepEqual(
    evidence.map((item) => item.command),
    ["npm test -- first", "npm test -- second"],
  );
});
...
test("long hook command scratch filenames never use an untrusted tool name", async () => {
  ...
  const env = {
    ...
    OTM_STORAGE: "json",
    OTM_STATE_DIR: fs.mkdtempSync(
      path.join(os.tmpdir(), "otm-hook-scratch-state-"),
    ),
    CODEX_THREAD_ID: "scratch-session",
    OTM_TRACK_MCP_EVIDENCE: "1",
  };
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  const started = manager.start({
    workspaceRoot,
    goal: "Capture long input safely",
    tasks: [{ title: "Inspect scratch evidence" }],
  });
  await runHookScript("post-tool-use", {
    cwd: workspaceRoot,
    env,
  ...
  const evidence = manager.store
    .getTask(started.snapshot.tasks[0].id)
    .evidence.at(-1);
  assert.match(evidence.notes.scratchFile, /-command-[a-f0-9]{12}\.txt$/);
  assert.doesNotMatch(evidence.notes.scratchFile, /outside|\.\./);
  assert.ok(
    fs.existsSync(path.join(workspaceRoot, evidence.notes.scratchFile)),
  );
});
