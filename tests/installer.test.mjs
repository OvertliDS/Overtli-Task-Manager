...
test('workspace installation dry-run creates no state directories or managed files', () => {
  ...
  const result = installWorkspace({ workspaceRoot, packageRoot, dryRun: true, installMcpConfig: true });
...
test('a late workspace installation failure rolls back every earlier managed file', () => {
  ...
  fs.mkdirSync(path.join(workspaceRoot, '.codex', 'config.toml'), { recursive: true });
  const result = installWorkspace({ workspaceRoot, packageRoot, installMcpConfig: true });
  assert.equal(result.ok, false);
  assert.equal(result.rolledBack, true);
  assert.equal(fs.existsSync(path.join(workspaceRoot, 'AGENTS.md')), false);
  assert.equal(fs.existsSync(path.join(workspaceRoot, '.codex', 'hooks.json')), false);
  ...
  assert.equal(fs.statSync(path.join(workspaceRoot, '.codex', 'config.toml')).isDirectory(), true);
});
...
test('malformed hooks JSON blocks installation before any managed file is written', () => {
  ...
  assert.equal(result.ok, false);
  assert.equal(result.results.find((item) => item.step === 'hooks').errorCode, 'HOOKS_JSON_INVALID');
  assert.equal(fs.readFileSync(hooksPath, 'utf8'), '{not valid json');
  assert.equal(fs.existsSync(path.join(workspaceRoot, 'AGENTS.md')), false);
  assert.equal(fs.existsSync(path.join(workspaceRoot, '.agents')), false);
  assert.equal(fs.existsSync(path.join(workspaceRoot, '.codex', 'overtli-task-manager', 'install.json')), false);
});
...
test('duplicate managed markers block installation before other managed files are written', () => {
  ...
  fs.writeFileSync(agentsPath, '<!-- OVERTLI-TASK-MANAGER:BEGIN v1 -->\nfirst\n<!-- OVERTLI-TASK-MANAGER:END -->\n<!-- OVERTLI-TASK-MANAGER:BEGIN v1 -->\nsecond\n<!-- OVERTLI-TASK-MANAGER:END -->\n', 'utf8');
  const result = installWorkspace({ workspaceRoot, packageRoot });
  assert.equal(result.ok, false);
  assert.equal(result.results.find((item) => item.step === 'agents').action, 'conflict');
  assert.equal(fs.existsSync(path.join(workspaceRoot, '.codex', 'hooks.json')), false);
