...
function sqliteTestEnv(name) {
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `${name}-sqlite-state-`),
  );
  const env = {
    ...process.env,
    OTM_STORAGE: "sqlite",
...
function internalStepTitles(task) {
  return (task.internalSteps || task.metadata?.internalSteps || []).map(
...
function internalStepStatuses(task) {
  return (task.internalSteps || task.metadata?.internalSteps || []).map(
    (step) => (typeof step === "string" ? "pending" : step.status),
  );
}

function finishInternalSteps(
  manager,
  workspaceRoot,
  taskId,
  statusByTitle = {},
  ...
  for (const step of task?.internalSteps ||
    ...
    manager.progress({
      ...
      evidence: {
        kind: "internal_step",
        summary: `Internal step terminal: ${title}`,
      },
    });
  }
}
...
test("route lifecycle requires evidence and clears after finalization", () => {
    ...
    tasks: [
      {
        title: "Create route",
        required: true,
        acceptanceCriteria: ["Route exists"],
      },
      {
        title: "Validate route",
        required: true,
        acceptanceCriteria: ["Audit passes after evidence"],
      },
    ],
  });
  ...
  assert.throws(
    () => manager.completeTask({ workspaceRoot, taskId: first.id }),
    /evidence is attached/,
  );
  finishInternalSteps(manager, workspaceRoot, first.id);
  const completedFirst = manager.completeTask({
    workspaceRoot,
    taskId: first.id,
    evidence: { kind: "manual_note", summary: "Route created." },
  });
  assert.match(completedFirst.markdown, /^### OTM Progress/);
  assert.equal(completedFirst.snapshot.lastRenderedMode, "delta");
  assert.equal(
    completedFirst.snapshot.tasks.find((task) => task.id === second.id).status,
    "active",
  );
  ...
  manager.completeTask({
    ...
    evidence: {
      kind: "test_result",
      summary: "Audit passed after both tasks.",
    },
  });
  ...
  assert.ok(
    fs.existsSync(
      path.join(workspaceRoot, ".codex/overtli-task-manager/current.json"),
    ),
  );
...
test("concurrent Codex sessions keep independent routes and current files in one workspace", () => {
  ...
  const managerA = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: sessionA },
  });
  const managerB = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: sessionB },
  });

  const startedA = managerA.start({
    workspaceRoot,
    goal: "Route A",
    tasks: [{ title: "Task A" }],
  });
  const startedB = managerB.start({
    workspaceRoot,
    replaceExisting: true,
    goal: "Route B",
    tasks: [{ title: "Task B" }],
  ...
  assert.equal(
    managerA.snapshot({ workspaceRoot, write: false }).run.id,
    startedA.run.id,
  );
  assert.equal(
    managerB.snapshot({ workspaceRoot, write: false }).run.id,
    startedB.run.id,
  );
  assert.notEqual(
    startedA.snapshot.paths.currentJson,
    startedB.snapshot.paths.currentJson,
  );
  assert.ok(
    fs.existsSync(
      path.join(workspaceRoot, startedA.snapshot.paths.currentJson),
    ),
  );
  assert.ok(
    fs.existsSync(
      path.join(workspaceRoot, startedB.snapshot.paths.currentJson),
    ),
  );
  ...
  assert.throws(
    ...
    { code: "CLEAR_REQUIRES_FINALIZATION" },
  );
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(currentJsonPath(workspaceRoot), "utf8"))
      .activeSessionCount,
    2,
  );
  managerA.abandonRun({
    ...
    reason: "Explicit test abandonment",
  });
  ...
  const runChild = (sessionId) =>
    new Promise((resolve, reject) => {
      const child = spawn(
        ...
        ["--input-type=module", "-e", script],
        {
          ...
          env: {
            ...
            OTM_STORAGE: "json",
            OTM_STATE_DIR: stateDir,
            OTM_TEST_WORKSPACE: workspaceRoot,
      ...
      child.on("exit", (code) =>
        code === 0
          ? resolve()
          : reject(new Error(`Child ${sessionId} exited ${code}: ${stderr}`)),
      );
    });

  await Promise.all([
    runChild("process-session-a"),
    runChild("process-session-b"),
  ]);

  const state = JSON.parse(
    fs.readFileSync(path.join(stateDir, "json", "state.json"), "utf8"),
  );
  assert.equal(state.runs.length, 2);
  assert.deepEqual(
    new Set(state.runs.map((run) => run.sessionId)),
    new Set(["process-session-a", "process-session-b"]),
  );
  const index = JSON.parse(
    fs.readFileSync(currentJsonPath(workspaceRoot), "utf8"),
  );
...
test("two simultaneous starts for one workspace and session create exactly one active route", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-process-single-scope-state-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  const script = `
    import { createTaskManager } from './src/core/manager.mjs';
...
test("legacy unscoped routes require explicit opt-in before a scoped session claims them", () => {
  ...
  const legacy = legacyManager.start({
    workspaceRoot,
    goal: "Legacy route",
  ...
  const managerA = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: "claiming-thread" },
  });
  ...
  const claimingManager = createTaskManager({
    ...
    env: {
      ...baseEnv,
      CODEX_THREAD_ID: "explicit-claiming-thread",
      OTM_CLAIM_LEGACY_ROUTE: "1",
    },
  });
  assert.equal(
    claimingManager.snapshot({ workspaceRoot, write: false }).run.id,
    legacy.run.id,
  );
  assert.equal(
    claimingManager.store.getRun(legacy.run.id).sessionId,
    "explicit-claiming-thread",
  );
});

test("explicit run ids enforce strict session scope without silently adopting legacy runs", () => {
  const workspaceRoot = tempWorkspace("otm-explicit-session-scope-");
  const env = testEnv("scope-owner");
  const owner = createTaskManager({
    cwd: workspaceRoot,
    env: { ...env, CODEX_THREAD_ID: "scope-owner" },
  });
  const scoped = owner.start({
    workspaceRoot,
    goal: "Scoped",
    tasks: [{ title: "Scoped task" }],
  });
  const noSession = createTaskManager({
    cwd: workspaceRoot,
    env: { ...env, CODEX_THREAD_ID: undefined },
  });
  const otherSession = createTaskManager({
    cwd: workspaceRoot,
    env: { ...env, CODEX_THREAD_ID: "scope-other" },
  });
  assert.throws(
    () =>
      noSession.snapshot({ workspaceRoot, runId: scoped.run.id, write: false }),
    { code: "SESSION_SCOPE_MISMATCH" },
  );
  assert.throws(
    () => otherSession.auditStop({ workspaceRoot, runId: scoped.run.id }),
    { code: "SESSION_SCOPE_MISMATCH" },
  );
  assert.equal(
    owner.snapshot({ workspaceRoot, runId: scoped.run.id, write: false }).run
      .id,
    scoped.run.id,
  );
});
...
test("session identity resolves supported hook payload aliases before environment fallback", () => {
  assert.equal(
    resolveSessionId({ session_id: "session-id" }, {}),
    "session-id",
  );
  assert.equal(resolveSessionId({ thread_id: "thread-id" }, {}), "thread-id");
  assert.equal(
    resolveSessionId({ conversationId: "conversation-id" }, {}),
    "conversation-id",
  );
  assert.equal(
...
test("sqlite isolates multiple chats in one project and one chat across projects", () => {
  ...
  const managerA1 = createTaskManager({
    cwd: workspaceA,
    env: { ...baseEnv, CODEX_THREAD_ID: "chat-a" },
  });
  const managerA2 = createTaskManager({
    cwd: workspaceA,
    env: { ...baseEnv, CODEX_THREAD_ID: "chat-b" },
  });
  const managerB1 = createTaskManager({
    cwd: workspaceB,
    env: { ...baseEnv, CODEX_THREAD_ID: "chat-a" },
  });
  const routeA1 = managerA1.start({
    workspaceRoot: workspaceA,
    goal: "A1",
    tasks: [{ title: "A1" }],
  });
  const routeA2 = managerA2.start({
    workspaceRoot: workspaceA,
    goal: "A2",
    tasks: [{ title: "A2" }],
  });
  const routeB1 = managerB1.start({
    workspaceRoot: workspaceB,
    goal: "B1",
    tasks: [{ title: "B1" }],
  });
  ...
  assert.equal(
    managerA1.snapshot({ workspaceRoot: workspaceA, write: false }).run.id,
    routeA1.run.id,
  );
  assert.equal(
    managerA2.snapshot({ workspaceRoot: workspaceA, write: false }).run.id,
    routeA2.run.id,
  );
  assert.equal(
...
test("unscoped snapshots preserve the workspace index when scoped routes are active", () => {
  ...
  const managerA = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: "index-a" },
  });
  const managerB = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: "index-b" },
  });
...
test("scoped state writes do not prune another session scratch evidence", () => {
  ...
  const managerA = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: "scratch-a" },
  });
  const managerB = createTaskManager({
    cwd: workspaceRoot,
    env: { ...baseEnv, CODEX_THREAD_ID: "scratch-b" },
  });
  const routeA = managerA.start({
    workspaceRoot,
    goal: "Scratch A",
    tasks: [{ title: "A" }],
  });
  ...
  managerA.progress({
    workspaceRoot,
    taskId: routeA.snapshot.tasks[0].id,
    message: "Write A state.",
  });
  assert.equal(fs.existsSync(evidencePath), true);
  createTaskManager({ cwd: workspaceRoot, env: baseEnv }).clearCurrent({
    workspaceRoot,
  });
  assert.equal(fs.existsSync(evidencePath), true);
});
...
test("current state writes use organized temp directory and clean stale state temp files", () => {
  ...
  const leaked = fs
    ...
    .filter((name) => /^current\.(?:md|json)\.\d+\.\d+\.tmp$/.test(name));
  assert.deepEqual(leaked, []);
  const tempLeaked = fs
    .readdirSync(workspaceTempDir(workspaceRoot))
...
test("default cleanup preserves active-session scratch and allSessions cleanup is explicit", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-explicit-clean"),
  });
  manager.start({
    workspaceRoot,
    replaceExisting: true,
  ...
  assert.equal(
    ...
    true,
    "cleanup dry run must preserve candidate files",
  );
  ...
  const forced = manager.cleanupWorkspace({
    workspaceRoot,
    allSessions: true,
    confirm: true,
    scratchMaxAgeMs: 0,
...
test("history pruning defaults to durable cleanup while preserving active routes", () => {
  ...
  const oldTask = manager.snapshot({ workspaceRoot, write: false }).snapshot
    .tasks[0];
  ...
  manager.store.updateRun(oldRun.id, {
    status: "cleared",
    updatedAt: oldIso,
    finalizedAt: oldIso,
  });
  ...
  manager.store.updateRun(activeRun.id, {
    status: "active",
    updatedAt: oldIso,
    finalizedAt: null,
  });

  const dryRun = manager.pruneHistory({
    workspaceRoot,
    now: recentNow,
    dryRun: true,
  });
  ...
  assert.equal(
    manager.store
      ...
      .some((summary) => summary.runId === oldRun.id),
    false,
  );
  assert.equal(
    manager.store
      .listCache(workspaceRoot, 20)
      .some((entry) => entry.id === "cache_old"),
    false,
  );
...
test("sqlite history pruning removes old inactive rows", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: sqliteTestEnv("otm-sqlite-prune"),
  });
  const oldIso = "2026-06-20T00:00:00.000Z";
  ...
  manager.finalizeTurn({ workspaceRoot, clear: true });
  manager.store.updateRun(started.run.id, {
    status: "cleared",
    updatedAt: oldIso,
    finalizedAt: oldIso,
  });
...
test("post-tool hook stores long raw command input in scratchpad instead of route evidence", async () => {
  ...
  const env = {
    ...testEnv("otm-scratchpad"),
    CODEX_THREAD_ID: "scratchpad-session",
  };
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  manager.start({
    workspaceRoot,
  ...
  assert.match(
    evidence.command,
    /^\[omitted long apply_patch input; saved to /,
  );
...
test("reconcile preserves model task order and activates newly added work by insertion order", () => {
  ...
  assert.equal(
    snapshot.tasks.find(
      (task) => task.title === "Run final audit and clear route",
    ).status,
    "active",
  );
  assert.deepEqual(
  ...
  const auditTaskId = snapshot.tasks.find(
    (task) => task.title === "Run final audit and clear route",
  ).id;
  ...
  manager.reconcile({
    ...
    tasks: [
      {
        title: "Fix status accuracy",
        required: true,
        acceptanceCriteria: ["Current task table matches header"],
      },
    ],
  });
  ...
  assert.equal(
    afterAudit.tasks.find((task) => task.title === "Update README docs").status,
    "active",
  );
  assert.equal(
    afterAudit.tasks.find((task) => task.title === "Fix status accuracy")
      .status,
    "pending",
  );
});
...
test("manual task switching is blocked until the active task is completed or reconciled", () => {
    ...
    changes: [
      {
        action: "activate",
        taskId: second.id,
        reason: "Explicit steering switch",
      },
    ],
  });
  ...
  assert.equal(
    switched.tasks.find((task) => task.id === second.id).status,
    "active",
  );
  assert.equal(
    switched.tasks.find((task) => task.id === first.id).status,
    "pending",
  );
});
...
test("explicit reconcile activation overrides current model order when the model steers route focus", () => {
  ...
  const docsTask = started.snapshot.tasks.find(
    (task) => task.title === "Update docs for route behavior",
  );

  assert.equal(
    started.snapshot.currentTaskTitle,
    "Globally reinstall latest package and hooks",
  );
    ...
    changes: [
      {
        action: "activate",
        taskId: docsTask.id,
        reason: "Model identified docs work before reinstall",
      },
    ],
  });
  ...
  assert.equal(
    steered.tasks.find((task) => task.id === docsTask.id).status,
    "active",
  );
  assert.equal(
    steered.tasks.find(
      (task) => task.title === "Globally reinstall latest package and hooks",
    ).status,
    "pending",
  );
});
...
test("reconcile merges related open tasks, adds distinct tasks, and stores internal substeps", () => {
  ...
  manager.start({
    ...
    tasks: [
      {
        title: "Optimize render behavior",
        required: true,
        acceptanceCriteria: ["Render policy is stable"],
      },
    ],
  });
  ...
  assert.deepEqual(tasks[1].acceptanceCriteria, [
    "Compact progress stays fast",
  ]);
  ...
  assert.match(
    manager.snapshot({ workspaceRoot, write: false }).markdown,
    /Profile current render path/,
  );
});
...
test("reconcile can explicitly reopen completed tasks without losing evidence", () => {
  ...
  manager.reconcile({
    workspaceRoot,
    changes: [
      { action: "reopen", taskId, reason: "User requested another docs pass" },
    ],
  });
  const reopened = manager
    .snapshot({ workspaceRoot, write: false })
    .snapshot.tasks.find((task) => task.id === taskId);
  assert.equal(reopened.status, "active");
  assert.ok(
    reopened.evidence.some(
      (item) => item.summary === "Initial docs check passed.",
    ),
  );
  assert.deepEqual(
    reopened.internalSteps.map((step) => step.status),
    ["active"],
  );
...
test("model-supplied route segments from rich prompt context preserve internal steps", () => {
  ...
  const started = manager.start({
    ...
    screenshots: [
      {
        description:
          "Screenshot shows Save button hidden behind footer and profile avatar overlapping the title.",
      },
    ],
    tasks: [
      {
        ...
        acceptanceCriteria: [
          "Save button is visible and usable in the affected profile screen state",
        ],
      },
      {
  ...
  assert.deepEqual(
    started.snapshot.tasks.map((task) => task.title),
    [
  ...
  assert.deepEqual(internalStepStatuses(started.snapshot.tasks[0]), [
    "active",
    "pending",
    "pending",
    "pending",
...
test("model-supplied route segments without internal steps get category-aware defaults", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-category-steps"),
  });
  const started = manager.start({
    workspaceRoot,
    replaceExisting: true,
...
test("internal step progress persists without completing the route gate", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-internal-steps"),
  });
  const started = manager.start({
    workspaceRoot,
    replaceExisting: true,
  ...
  const progress = manager.progress({
    ...
    evidence: {
      kind: "manual_note",
      summary: "Internal source inspection complete.",
    },
  });
  ...
  assert.equal(
    progress.snapshot.currentInternalStep.title,
    "Patch progress updates",
  );
  assert.deepEqual(
    task.internalSteps.map((step) => step.status),
    ["done", "active", "pending"],
  );
  ...
  assert.throws(
    () => manager.completeTask({ workspaceRoot, taskId }),
    /evidence is attached/,
  );
  assert.throws(
    () =>
      manager.completeTask({
        ...
        evidence: {
          kind: "test_result",
          summary: "Evidence is present but internal steps are unfinished.",
        },
      }),
  ...
  const completed = manager.completeTask({
    ...
    evidence: {
      kind: "test_result",
      summary: "Top-level gate completed with evidence.",
    },
  });
  const completedTask = completed.snapshot.tasks.find(
    (item) => item.id === taskId,
  );
...
test("workspace installer is idempotent and preserves existing guidance", () => {
  ...
  assert.ok(
    fs.existsSync(
      path.join(workspaceRoot, ".agents/skills/overtli-task-manager/SKILL.md"),
    ),
  );
  assert.ok(fs.existsSync(path.join(workspaceRoot, ".codex/hooks.json")));
  const hooks = JSON.parse(
    fs.readFileSync(path.join(workspaceRoot, ".codex/hooks.json"), "utf8"),
  ).hooks;
  ...
  assert.equal(
    hooks.SessionStart.at(-1).hooks[0].command,
    `node "${expectedCli}" hook session-start`,
  );
  assert.equal(
    hooks.UserPromptSubmit.at(-1).hooks[0].command,
    `node "${expectedCli}" hook user-prompt-submit`,
  );
...
test("workspace installer patches AGENTS.md by default and only patches override explicitly", () => {
  ...
  assert.match(
    firstAgents.warning,
    /AGENTS\.override\.md exists and was not patched/,
  );
  assert.match(
    fs.readFileSync(path.join(workspaceRoot, "AGENTS.md"), "utf8"),
    /OVERTLI-TASK-MANAGER:BEGIN/,
  );
  assert.doesNotMatch(
    fs.readFileSync(path.join(workspaceRoot, "AGENTS.override.md"), "utf8"),
    /OVERTLI-TASK-MANAGER:BEGIN/,
  );

  const explicit = installWorkspace({
    workspaceRoot,
    packageRoot,
    targetAgentsFile: "AGENTS.override.md",
    dryRun: false,
  });
  const explicitAgents = explicit.results.find(
    (item) => item.step === "agents",
  );
...
test("skill installer manages every packaged file in a skill directory", () => {
  ...
  const packageRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-skill-package-"),
  );
  ...
  fs.writeFileSync(
    path.join(sourceSkill, "references", "guide.md"),
    "Reference\n",
    "utf8",
  );
...
test("workspace uninstall is previewable, confirmation-gated, and preserves unrelated configuration", () => {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ hooks: { Stop: [{ matcher: "keep", hooks: [{ type: "command", command: "keep-existing-hook" }] }] } }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
  ...
  assert.equal(
    installWorkspace({ workspaceRoot, packageRoot, installMcpConfig: true }).ok,
    true,
  );
  const agentsPath = path.join(workspaceRoot, "AGENTS.md");
  const beforePreview = fs.readFileSync(agentsPath, "utf8");
  const preview = uninstallWorkspace({
    workspaceRoot,
    packageRoot,
    dryRun: true,
  });
  ...
  assert.equal(
    fs.readFileSync(path.join(workspaceRoot, ".gitignore"), "utf8").trim(),
    "node_modules/",
  );
  assert.match(
  ...
  assert.doesNotMatch(
    fs.readFileSync(path.join(workspaceRoot, ".codex", "config.toml"), "utf8"),
    /OVERTLI-TASK-MANAGER:MCP:BEGIN/,
  );
  const hooks = JSON.parse(
    fs.readFileSync(path.join(workspaceRoot, ".codex", "hooks.json"), "utf8"),
  ).hooks;
  ...
  assert.equal(
    fs.existsSync(
      path.join(workspaceRoot, ".agents", "skills", "overtli-task-manager"),
    ),
    false,
  );
...
test("workspace uninstall preserves user-modified managed skill directories", () => {
  ...
  fs.writeFileSync(
    path.join(skillDir, "local-notes.md"),
    "do not remove\n",
    "utf8",
  );
  const result = uninstallWorkspace({
  ...
  assert.equal(
    skillResult.installed.find((item) => item.name === "overtli-task-manager")
      .action,
    "skipped-ownership-mismatch",
  );
  assert.equal(
    fs.readFileSync(path.join(skillDir, "local-notes.md"), "utf8"),
    "do not remove\n",
  );
});
...
test("workspace uninstall preflight blocks all writes for malformed hooks and state removal keeps recovery data", () => {
  ...
  const blocked = uninstallWorkspace({
    workspaceRoot,
    packageRoot,
    confirm: true,
  });
  ...
  const activeBlocked = uninstallWorkspace({
    workspaceRoot,
    packageRoot,
    confirm: true,
    removeState: true,
  ...
  assert.equal(
    activeBlocked.results.find((item) => item.step === "state").action,
    "active-state",
  );
  fs.writeFileSync(
    activeCurrent,
  ...
  const removed = uninstallWorkspace({
    workspaceRoot,
    packageRoot,
    confirm: true,
    removeState: true,
  ...
  assert.equal(
    fs.existsSync(path.join(workspaceRoot, ".codex", "overtli-task-manager")),
    false,
  );
  assert.ok(
    fs.existsSync(
      path.join(
        ...
        path.basename(removed.backupPath),
        "uninstall.json",
      ),
    ),
  );
...
test("global installer preserves unrelated hooks and installs discoverable skills", () => {
  const codexHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-global-install-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  fs.writeFileSync(
    path.join(codexHome, "hooks.json"),
    `${JSON.stringify({ hooks: { Stop: [{ hooks: [{ type: "command", command: "keep-existing-hook" }] }] } }, null, 2)}\n`,
    "utf8",
  );

  const first = installGlobal({
    codexHome,
    packageRoot,
  ...
  assert.equal(
    ...
    path.join(
      codexHome,
      "hooks.json.before-otm-global-2026-07-04T12-00-00-000Z.bak",
    ),
  );
  assert.ok(fs.existsSync(first.backupPath));
  assert.ok(
    fs.existsSync(path.join(first.transactionBackupPath, "install.json")),
  );
  assert.equal(
    second.results.find((item) => item.step === "hooks").action,
    "unchanged",
  );
  assert.equal(
    second.results.find((item) => item.step === "skills").action,
    "unchanged",
  );
  ...
  assert.equal(hooks.Stop[0].hooks[0].command, "keep-existing-hook");
  assert.equal(
    hooks.Stop.at(-1).hooks[0].command,
    `node "${path.join(packageRoot, "bin", "otm.mjs")}" hook stop`,
  );
  assert.ok(
    fs.existsSync(
      path.join(codexHome, "skills", "overtli-task-manager", "SKILL.md"),
    ),
  );
});
...
test("global uninstall is previewable, confirmation-gated, and preserves unrelated hooks", () => {
  const codexHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-global-uninstall-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  ...
  assert.equal(
    uninstallGlobal({ codexHome, packageRoot }).results[0].action,
  ...
  assert.equal(hooks.Stop[0].hooks[0].command, "keep-existing-hook");
  assert.equal(
    fs.existsSync(path.join(codexHome, "skills", "overtli-task-manager")),
    false,
  );
  assert.ok(fs.existsSync(path.join(result.backupPath, "manifest.json")));
});
...
test("global uninstall preserves modified skills and malformed hooks block all writes", () => {
  const codexHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-global-uninstall-safety-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  assert.equal(installGlobal({ codexHome, packageRoot }).ok, true);
  ...
  assert.equal(
    preserved.results
      .find((item) => item.step === "skills")
      .installed.find((item) => item.name === "overtli-task-manager").action,
    "skipped-ownership-mismatch",
  );
  assert.equal(
    fs.readFileSync(path.join(skillDir, "local.md"), "utf8"),
    "retain\n",
  );

  const blockedHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-global-uninstall-invalid-"),
  );
  ...
  const blocked = uninstallGlobal({
    codexHome: blockedHome,
    packageRoot,
    confirm: true,
  });
  assert.equal(blocked.ok, false);
  assert.equal(
    fs.existsSync(path.join(blockedHome, "skills", "overtli-task-manager")),
    true,
  );
...
test("global installer preflight blocks every write when hooks configuration is malformed", () => {
  const codexHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-global-preflight-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
...
test("a late global skill installation failure restores the earlier hooks change", () => {
  const codexHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-global-rollback-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  const hooksPath = path.join(codexHome, "hooks.json");
  const originalHooks = '{\n  "hooks": {}\n}\n';
  ...
  assert.equal(
    fs.readFileSync(path.join(codexHome, "skills"), "utf8"),
    "not a directory",
  );
});
...
test("postinstall never mutates global Codex state without explicit opt-in", () => {
  const codexHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-postinstall-home-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  const activePluginRoot = path.join(
    codexHome,
    "plugins",
    "overtli-task-manager",
  );
  ...
  assert.equal(
    shouldAutoInstallGlobal({ packageRoot, codexHome, env: {} }),
    false,
  );
  assert.equal(
    shouldAutoInstallGlobal({
  ...
  assert.equal(
    shouldAutoInstallGlobal({
      packageRoot: activePluginRoot,
      codexHome,
      env: { OTM_AUTO_INSTALL_GLOBAL: "0" },
    }),
    false,
  );

  const skipped = runPostinstall({
    packageRoot,
  ...
  const installed = runPostinstall({
    packageRoot,
    env: { CODEX_HOME: codexHome, OTM_AUTO_INSTALL_GLOBAL: "1" },
  });
...
test("session start creates and refreshes only the managed AGENTS.md block", async () => {
  ...
  const created = await withCapturedStdout(() =>
    runHookScript("session-start", {
      ...
      stdin: JSON.stringify({
        ...
        invocation_id: "agents-create",
      }),
    }),
  );
  assert.match(
    fs.readFileSync(path.join(emptyWorkspace, "AGENTS.md"), "utf8"),
    /OVERTLI-TASK-MANAGER:BEGIN/,
  );
  assert.match(
    created.result.systemMessage,
    /AGENTS\.md managed instructions: created/,
  );
  ...
  const first = await withCapturedStdout(() =>
    runHookScript("session-start", {
      cwd: workspaceRoot,
      env,
      stdin: JSON.stringify({
  ...
  assert.equal(
    (firstAgents.match(/OVERTLI-TASK-MANAGER:BEGIN/g) || []).length,
    1,
  );
  assert.match(
    first.result.systemMessage,
    /AGENTS\.md managed instructions: appended/,
  );

  const outdated = firstAgents.replace(
    "Prefer thorough completion over shallow progress.",
    "Outdated managed instruction.",
  );
  fs.writeFileSync(agentsPath, outdated, "utf8");
  const second = await withCapturedStdout(() =>
    runHookScript("session-start", {
      cwd: workspaceRoot,
      env,
      stdin: JSON.stringify({
  ...
  assert.equal(
    (refreshed.match(/OVERTLI-TASK-MANAGER:BEGIN/g) || []).length,
    1,
  );
  assert.match(
    second.result.systemMessage,
    /AGENTS\.md managed instructions: updated/,
  );
});
  ...
  const emptyResult = await withCapturedStdout(() =>
    runHookScript("session-start", {
      ...
      stdin: JSON.stringify({
        cwd: emptyWorkspace,
        hook_event_name: "SessionStart",
        invocation_id: "agents-default-empty",
      }),
    }),
  );
...
test("session-start hook surfaces project-review diagnostics without blocking the session", async () => {
  ...
  const env = {
    ...testEnv("otm-hook-review-diagnostic"),
    CODEX_THREAD_ID: "review-diagnostic",
    OTM_PROJECT_REVIEW_MAX_FILES: "0",
  ...
  assert.match(
    ...
    /Project-review diagnostic:.*maxFiles/i,
  );
});
...
test("project review scans memory-bank / memory_bank and indexes overview files", () => {
  ...
  fs.writeFileSync(
    path.join(mbDir, "projectbrief.md"),
  ...
  fs.writeFileSync(
    path.join(mbHyphenDir, "productContext.md"),
  ...
  const briefSource = review.sources.find(
    (s) => s.path.replace(/\\/g, "/") === "memory_bank/projectbrief.md",
  );
  const contextSource = review.sources.find(
    (s) => s.path.replace(/\\/g, "/") === "memory-bank/productContext.md",
  );
...
test("project review counts only eligible files, reports limits, and cannot follow an external symlink", () => {
  ...
  const external = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-review-external-"),
  );
  fs.writeFileSync(
...
test("separate finalize and clear recover the completed scoped run and mark its summary cleared", () => {
  ...
  const env = {
    ...testEnv("otm-finalize-clear-state"),
    CODEX_THREAD_ID: "finalize-clear-session",
  };
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  const started = manager.start({
    workspaceRoot,
    goal: "Finalize then clear",
    tasks: [{ title: "Finish" }],
  });
  const taskId = started.snapshot.tasks[0].id;
  finishInternalSteps(manager, workspaceRoot, taskId);
  manager.completeTask({
    workspaceRoot,
    taskId,
    evidence: { kind: "test_result", summary: "Complete." },
  });
...
test("finalization retries reuse one deterministic summary and memory record", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-finalize-retry"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Retry finalization",
    tasks: [{ title: "Complete task" }],
  });
  finishInternalSteps(manager, workspaceRoot, started.snapshot.tasks[0].id);
  manager.completeTask({
    workspaceRoot,
    taskId: started.snapshot.tasks[0].id,
    evidence: { kind: "test_result", summary: "Passed." },
  });
  const first = manager.finalizeTurn({
    workspaceRoot,
    operationId: "finalize-once",
  });
  const second = manager.finalizeTurn({
    workspaceRoot,
    runId: first.run.id,
    operationId: "finalize-once",
  });
  ...
  assert.equal(
    manager.store
      .listSummaries(workspaceRoot, 20)
      .filter((summary) => summary.runId === first.run.id).length,
    1,
  );
  assert.equal(
    manager.store
      .listCache(workspaceRoot, 100)
      .filter((entry) => entry.kind === "turn_summary").length,
    1,
  );
});
...
test("repair republishs missing summary files from durable summary records", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-summary-repair"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Repair summary",
  ...
  const finalized = manager.finalizeTurn({
    workspaceRoot,
    operationId: "repair-summary",
  });
  const files = fs.readdirSync(
    path.join(workspaceRoot, ".codex", "overtli-task-manager", "summaries"),
  );
  ...
  assert.equal(
    manager.repairSummaries({ workspaceRoot, dryRun: true }).repaired,
    0,
  );
...
test("incomplete finalization requires and records an explicit reason", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-incomplete-finalize"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Incomplete checkpoint",
    tasks: [{ title: "Unfinished task" }],
  });
  assert.throws(
    () =>
    ...
    { code: "INCOMPLETE_FINALIZATION_REASON_REQUIRED" },
  );
  const finalized = manager.finalizeTurn({
    workspaceRoot,
    runId: started.run.id,
    allowIncomplete: true,
    reason: "Waiting for external access.",
  });
  assert.equal(finalized.run.status, "blocked");
  assert.equal(
    finalized.run.metadata.incompleteFinalizationReason,
...
test("blocked routes resume through an explicit transition and finalized routes archive idempotently", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-resume-archive"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Resume then archive",
    tasks: [{ title: "Recover task" }],
  });
  const taskId = started.snapshot.tasks[0].id;
  const blocked = manager.blockTask({
    workspaceRoot,
    taskId,
    reason: "Waiting for a dependency",
  });
  assert.equal(blocked.run.status, "blocked");
  const resumed = manager.resumeRun({
    workspaceRoot,
  ...
  manager.completeTask({
    workspaceRoot,
    taskId,
    evidence: { kind: "test_result", summary: "Recovered and verified" },
  });
  const finalized = manager.finalizeTurn({
    workspaceRoot,
    operationId: "archive-route",
  });
  const archived = manager.archiveRun({
    workspaceRoot,
    runId: finalized.run.id,
    reason: "Retain as history",
  });
  assert.equal(archived.run.status, "archived");
  assert.equal(
    manager.archiveRun({ workspaceRoot, runId: finalized.run.id }).idempotent,
    true,
  );
  assert.throws(
    () => manager.resumeRun({ workspaceRoot, runId: finalized.run.id }),
...
test("identity-less hooks cannot read or mutate legacy route state", async () => {
  ...
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  const legacy = manager.start({
    workspaceRoot,
    goal: "Legacy private route",
    tasks: [{ title: "Do not expose this task" }],
  });
  const before = manager.store.getEvents(legacy.run.id).length;

  const sessionStart = await withCapturedStdout(() =>
    runHookScript("session-start", {
      cwd: workspaceRoot,
      env,
      stdin: JSON.stringify({
        cwd: workspaceRoot,
        hook_event_name: "SessionStart",
        invocation_id: "unscoped-start",
      }),
    }),
  );
  const postTool = await withCapturedStdout(() =>
    runHookScript("post-tool-use", {
      cwd: workspaceRoot,
      env,
      stdin: JSON.stringify({
        ...
        invocation_id: "unscoped-tool",
        tool_name: "Bash",
        tool_response: { exit_code: 1 },
      }),
    }),
  );
  const stop = await withCapturedStdout(() =>
    runHookScript("stop", {
      cwd: workspaceRoot,
      env,
      stdin: JSON.stringify({
  ...
  assert.doesNotMatch(
    ...
    /Legacy private route|Do not expose this task/,
  );
...
test("separate global and workspace hook processes claim one shared invocation", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-hook-process-state-"),
  );
  const cliPath = fileURLToPath(new URL("../bin/otm.mjs", import.meta.url));
  const input = JSON.stringify({
    cwd: workspaceRoot,
    hook_event_name: "UserPromptSubmit",
    turn_id: "process-dedupe-turn",
    session_id: "process-dedupe-session",
  ...
  const runHook = () =>
    new Promise((resolve, reject) => {
      const child = spawn(
        process.execPath,
        [cliPath, "hook", "user-prompt-submit"],
        {
          cwd: workspaceRoot,
          env: {
            ...process.env,
            OTM_STORAGE: "json",
            OTM_STATE_DIR: stateDir,
            CODEX_THREAD_ID: "process-dedupe-session",
          },
  ...
  assert.equal(
    outputs.filter((item) => item.hookSpecificOutput?.additionalContext).length,
    1,
  );
  assert.equal(
    outputs.filter((item) => item.continue === true && !item.hookSpecificOutput)
      .length,
    1,
  );
});
  ...
  assert.throws(
    () =>
      manager.reconcile({
        ...
        runId: second.run.id,
        changes: [
          { action: "supersede", taskId: firstTaskId, reason: "Bad scope" },
        ],
      }),
...
test("reconciliation rejects terminal task input and does not fuzzy-merge distinct tasks", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-reconcile-input"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Reconcile input",
    tasks: [{ title: "Review API error contract" }],
  });
  assert.throws(
    () =>
      manager.reconcile({
        ...
        runId: started.run.id,
        tasks: [
          { title: "Review API error contract carefully", status: "done" },
        ],
      }),
  ...
  const result = manager.reconcile({
    workspaceRoot,
    runId: started.run.id,
    tasks: [{ title: "Review API error contract carefully" }],
  });
...
test("reconciliation commits replacements and additions through one revisioned mutation", () => {
  for (const env of [
    testEnv("otm-reconcile-atomic-json"),
    sqliteTestEnv("otm-reconcile-atomic-sqlite"),
  ]) {
    const workspaceRoot = tempWorkspace("otm-reconcile-atomic-");
    const manager = createTaskManager({ cwd: workspaceRoot, env });
    const started = manager.start({
      workspaceRoot,
      goal: "Atomically reconcile",
      tasks: [
    ...
    assert.equal(
      tasks.find((task) => task.id === "first").status,
      "superseded",
    );
    assert.equal(
      tasks.find((task) => task.id === "second").status,
      "superseded",
    );
    assert.equal(
      tasks.find((task) => task.id === "replacement").status,
      "active",
    );
    assert.equal(
      manager.store
        .getEvents(result.run.id, 100)
        .filter(
          (event) =>
            event.eventType === "run_reconciled" &&
            event.idempotencyKey === "atomic-reconcile-operation",
        ).length,
      1,
    );
    manager.store.close?.();
...
test("task dependencies reject missing and cyclic graphs and enforce sequential activation", () => {
  ...
  assert.throws(
    () =>
      manager.start({
        workspaceRoot,
        goal: "Missing",
        tasks: [{ id: "a", title: "A", dependsOn: ["missing"] }],
      }),
  ...
  assert.throws(
    () =>
      manager.markTaskActive({ workspaceRoot, taskId: "b", allowSwitch: true }),
...
test("reopening an internal step clears its stale completion timestamp", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-step-reopen"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Step reopen",
    tasks: [{ title: "Task", internalSteps: ["Step"] }],
  });
  const taskId = started.snapshot.tasks[0].id;
  manager.progress({
    workspaceRoot,
  ...
  assert.equal(
    manager.store.getTask(taskId).metadata.internalSteps[0].completedAt,
    undefined,
  );
});
...
test("mutations reject stale expected route revisions with the current revision", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-revision-conflict"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Revision",
  ...
  assert.throws(
    ...
    (error) =>
      error.code === "REVISION_CONFLICT" && error.details.currentRevision === 1,
  );
});
...
test("every task lifecycle mutation advances the durable route revision", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-revision-advance"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Revision advance",
    tasks: [{ title: "Task" }],
  });
  const taskId = started.snapshot.tasks[0].id;
  const first = manager.progress({
    workspaceRoot,
    taskId,
    expectedRevision: 1,
    message: "Checkpoint",
...
test("CLI uninstall dry-run performs no store or workspace mutations", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-cli-uninstall-state-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  ...
  try {
    const outcome = await handleCli({
      argv: ["uninstall", "--dry-run", "--json"],
      cwd: workspaceRoot,
      stdin: "",
      packageRoot,
...
test("CLI status, list-runs, and history support JSON operational output", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-cli-operations-state-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  const env = {
    ...process.env,
    OTM_STORAGE: "json",
    OTM_STATE_DIR: stateDir,
    CODEX_THREAD_ID: "cli-session",
  };
  const manager = createTaskManager({ cwd: workspaceRoot, env });
  const started = manager.start({
    workspaceRoot,
    goal: "CLI operations",
    tasks: [{ title: "Inspect history" }],
  });
  const originalLog = console.log;
  const output = [];
  console.log = (value) => output.push(String(value));
  try {
    ...
    await handleCli({
      argv: ["history", "--run-id", started.run.id, "--json"],
      cwd: workspaceRoot,
      stdin: "",
      packageRoot,
      env,
...
test("CLI backup and confirmed restore protect JSON durable state", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-cli-backup-state-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  const env = { ...process.env, OTM_STORAGE: "json", OTM_STATE_DIR: stateDir };
  ...
  try {
    ...
    await handleCli({
      argv: ["restore", "--input", backupPath, "--confirm"],
      cwd: workspaceRoot,
      stdin: "",
      packageRoot,
      env,
...
test("CLI SQLite backup produces a consistent queryable database image", async () => {
  ...
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "otm-cli-sqlite-backup-state-"),
  );
  const packageRoot = fileURLToPath(new URL("..", import.meta.url));
  const env = {
    ...process.env,
  ...
  try {
    assert.equal(
      copy.prepare("SELECT id FROM runs WHERE id = ?").get(started.run.id).id,
      started.run.id,
    );
  } finally {
    copy.close();
...
test("historical export and import preserve terminal workspace records but reject active routes", async () => {
  ...
  source.completeTask({
    workspaceRoot,
    taskId: task.id,
    evidence: { kind: "test_result", summary: "terminal evidence" },
  });
  ...
  const outputPath = path.join(
    os.tmpdir(),
    `otm-export-${Date.now()}-${Math.random()}.json`,
  );
  const logged = [];
  const originalLog = console.log;
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(outputPath, "utf8")).runs[0].id,
    document.runs[0].id,
  );
  ...
  assert.equal(
    target.listRuns({ workspaceRoot }).runs[0].id,
    document.runs[0].id,
  );
  assert.equal(
    target.inspectMemory({ workspaceRoot, id: "export-memory" }).entry.title,
    "Exported memory",
  );
  assert.throws(() => target.importHistorical({ workspaceRoot, document }), {
    code: "IMPORT_CONFLICT",
  });

  const activeDocument = source.exportWorkspace({ workspaceRoot });
  activeDocument.runs[0] = {
    ...activeDocument.runs[0],
    status: "active",
    finalizedAt: null,
  };
  assert.throws(
    () => target.importHistorical({ workspaceRoot, document: activeDocument }),
    { code: "IMPORT_ACTIVE_RUN_FORBIDDEN" },
  );
});
...
test("evidence redacts common credentials before durable storage", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-redaction"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Redact evidence",
    tasks: [{ title: "Check evidence" }],
  });
  const taskId = started.snapshot.tasks[0].id;
  manager.progress({
    workspaceRoot,
    ...
    evidence: {
      ...
      command:
        'curl -H "Authorization: Bearer abcdefghijklmnop" --api-key=very-secret',
      notes: { password: "not-safe", visible: "safe" },
    },
  });
  const evidence = manager.store.getTask(taskId).evidence.at(-1);
  assert.doesNotMatch(
    JSON.stringify(evidence),
    /secret-value|abcdefghijklmnop|not-safe/,
  );
...
test("both storage backends reject unscoped cache deletion", () => {
  const json = new JsonStore({
    stateDir: fs.mkdtempSync(
      path.join(os.tmpdir(), "otm-json-delete-selector-"),
    ),
  });
  json.init();
  assert.throws(() => json.deleteCache({}), {
  ...
  const sqlite = new SqliteStore({
    stateDir: fs.mkdtempSync(
      path.join(os.tmpdir(), "otm-sqlite-delete-selector-"),
    ),
  });
  sqlite.init();
  assert.throws(() => sqlite.deleteCache({}), {
    code: "CACHE_SELECTOR_REQUIRED",
  });
  sqlite.close();
});
...
test("JSON and SQLite commit run mutations atomically with revision conflict protection", () => {
  const stores = [];
  const json = new JsonStore({
    stateDir: fs.mkdtempSync(path.join(os.tmpdir(), "otm-json-atomic-")),
  });
  json.init();
  stores.push(json);
  const sqlite = new SqliteStore({
    stateDir: fs.mkdtempSync(path.join(os.tmpdir(), "otm-sqlite-atomic-")),
  });
  sqlite.init();
  stores.push(sqlite);
  for (const store of stores) {
    ...
    store.createRoute({
      ...
      tasks: [task],
      event: {
        id: `event-start-${store.kind}`,
        runId: run.id,
        turnId: null,
        hookEventName: null,
        eventType: "run_started",
        idempotencyKey: `start-${store.kind}`,
        payload: {},
        createdAt: now,
      },
    });
    ...
    store.commitRunMutation({
      ...
      tasks: [nextTask],
      event: {
        id: `event-block-${store.kind}`,
        runId: run.id,
        turnId: null,
        hookEventName: null,
        eventType: "task_blocked",
        idempotencyKey: `block-${store.kind}`,
        payload: {},
        createdAt: now,
      },
    });
...
test("memory list and inspect exclude expired entries while explicit purge reports exact counts", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-memory-operations"),
  });
  ...
  assert.deepEqual(
    manager.listMemory({ workspaceRoot }).entries.map((entry) => entry.id),
    ["live"],
  );
  assert.equal(
    manager.inspectMemory({ workspaceRoot, id: "live" }).entry.title,
    "Live memory",
  );
  assert.deepEqual(
    manager.purgeExpiredMemory({ workspaceRoot, dryRun: true }).matched,
    1,
  );
  assert.equal(manager.purgeExpiredMemory({ workspaceRoot }).deleted, 1);
  assert.throws(() => manager.inspectMemory({ workspaceRoot, id: "expired" }), {
...
test("workspace-wide memory deletion requires all:true and confirmation while preview reports exact matches", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-memory-delete-all"),
  });
  manager.upsertMemory({
    workspaceRoot,
    id: "one",
  ...
  assert.throws(() => manager.deleteMemory({ workspaceRoot, all: true }), {
    code: "MEMORY_DELETE_CONFIRMATION_REQUIRED",
  });
  const preview = manager.deleteMemory({
    workspaceRoot,
    all: true,
    confirm: true,
    dryRun: true,
  ...
  assert.equal(
    manager.deleteMemory({ workspaceRoot, all: true, confirm: true }).deleted,
    2,
  );
...
test("memory search ranks normalized title, tag, body, and score-hint matches", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-memory-search-ranking"),
  });
  manager.upsertMemory({
    workspaceRoot,
    id: "low",
  ...
  manager.upsertMemory({
    ...
    body: "Database migration validation evidence",
    tags: ["database", "migration"],
    scoreHint: 5,
  });
  const result = manager.searchMemory({
    workspaceRoot,
  ...
  assert.equal(
    result.entries.some((entry) => entry.id === "low"),
    true,
  );
  assert.ok(result.entries[0].matchReasons.includes("exact phrase"));
  assert.ok(
    result.entries[0].matchReasons.some((reason) =>
      reason.startsWith("title:"),
    ),
  );
  assert.ok(
    result.entries[0].matchReasons.some((reason) => reason.startsWith("tags:")),
  );
});
...
test("SQLite cache upserts preserve createdAt instead of replacing the row", () => {
  const store = new SqliteStore({
    stateDir: fs.mkdtempSync(
      path.join(os.tmpdir(), "otm-sqlite-cache-upsert-"),
    ),
  });
  store.init();
  const first = {
  ...
  assert.equal(
    store.listCache(first.workspaceRoot)[0].createdAt,
    first.createdAt,
  );
  store.close();
});
...
test("persistent checklist order stays planned and completion delta names the changed task", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: testEnv("otm-render-order"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Render order",
    tasks: [
      { title: "First", internalSteps: ["Verify first"] },
      { title: "Second", internalSteps: ["Verify second"] },
    ],
  });
  ...
  manager.progress({
    ...
    internalStepIndex: 0,
    internalStepStatus: "done",
    message: "First verified",
  });
  const completed = manager.completeTask({
    workspaceRoot,
    taskId: firstId,
    evidence: { kind: "test", summary: "First passed" },
  });
  assert.deepEqual(
    completed.snapshot.tasks.map((task) => task.title),
    ["First", "Second"],
  );
