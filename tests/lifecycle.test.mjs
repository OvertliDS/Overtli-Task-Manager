...
function completeAllStepsAndTask(manager, workspaceRoot, taskId, scope = {}) {
  ...
  manager.completeTask({
    workspaceRoot,
    taskId,
    evidence: { kind: "test_result", summary: "Verified completion." },
    ...scope,
  });
}
...
test("terminal task transitions terminalize internal steps and reject stale progress", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: env("otm-terminal-transitions"),
  });
  ...
  assert.deepEqual(internalStepStatuses(manager.store.getTask(droppedId)), [
    "skipped",
    "skipped",
  ]);
  const next = manager
    .snapshot({ workspaceRoot, write: false })
    .snapshot.tasks.find((task) => task.title === "Keep this");
...
test("public route creation rejects terminal task states and duplicate task identifiers", () => {
  ...
  assert.throws(
    () =>
      manager.start({
        workspaceRoot,
        goal: "Unsafe start",
        tasks: [{ title: "Bypass", status: "done" }],
      }),
  ...
  assert.throws(
    () =>
      manager.start({
        workspaceRoot,
        goal: "Oversized title",
        tasks: [{ title: "x".repeat(501) }],
      }),
  ...
  assert.throws(
    () =>
      manager.start({
        ...
        tasks: [{ title: "Task", priority: 1_001 }],
      }),
    { code: "INVALID_INPUT" },
  );
  assert.throws(
...
test("completion cannot be forced and completed tasks must reopen before drop", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: env("otm-transition-guards"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Transition guards",
    tasks: [{ title: "Guarded", internalSteps: ["Verify"] }],
  });
  ...
  assert.throws(
    () => manager.completeTask({ workspaceRoot, taskId, force: true }),
    /Forced completion/,
  );
  manager.completeTask({
    workspaceRoot,
  ...
  assert.throws(
    () =>
      manager.dropTask({ workspaceRoot, taskId, reason: "Rewrite history" }),
    /explicitly reopened/,
  );
  assert.equal(
...
test("ordinary activation cannot bypass the explicit blocked-task resume transition", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: env("otm-blocked-activation"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Resume guarded task",
  ...
  manager.blockTask({
    workspaceRoot,
    taskId,
    reason: "Awaiting an external dependency.",
  });
  assert.throws(() => manager.markTaskActive({ workspaceRoot, taskId }), {
...
test("direct lifecycle evidence applies the same bounded validation as MCP input", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: env("otm-evidence-bounds"),
  });
  const started = manager.start({
    workspaceRoot,
    goal: "Validate direct evidence",
    tasks: [{ title: "Record bounded evidence" }],
  });
  const taskId = started.snapshot.tasks[0].id;
  assert.throws(
    () =>
      manager.progress({
        ...
        taskId,
        message: "Too large",
        evidence: { kind: "test_result", summary: "x".repeat(16_001) },
      }),
    { code: "INPUT_TOO_LARGE" },
  );
  assert.throws(
    () =>
      manager.progress({
        ...
        evidence: {
          kind: "test_result",
          summary: "bounded",
          files: Array.from({ length: 129 }, (_, index) => `file-${index}`),
        },
      }),
    { code: "INPUT_TOO_LARGE" },
  );
  assert.throws(
    () =>
      manager.progress({
        ...
        evidence: { kind: "test_result", summary: "bounded", exitCode: 256 },
      }),
    { code: "INVALID_INPUT" },
  );
});
...
test("different finalized runs with the same goal retain independent turn-summary memory", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: env("otm-summary-memory-identity"),
  });
  ...
  manager.finalizeTurn({
    workspaceRoot,
    sessionId: "first",
    operationId: "first-finalization",
  });
  ...
  manager.finalizeTurn({
    workspaceRoot,
    sessionId: "second",
    operationId: "second-finalization",
  });
  const summaries = manager.store
    .listCache(workspaceRoot, 100)
    .filter((entry) => entry.kind === "turn_summary");
  assert.equal(summaries.length, 2);
  assert.equal(new Set(summaries.map((entry) => entry.id)).size, 2);
  assert.deepEqual(
    new Set(summaries.map((entry) => entry.source.runId)),
...
test("clear reports a redacted history-maintenance failure instead of silently discarding it", () => {
  ...
  const manager = createTaskManager({
    cwd: workspaceRoot,
    env: env("otm-clear-maintenance-warning"),
  });
  const started = manager.start({
    workspaceRoot,
  ...
  manager.store.pruneHistory = () => {
    throw new Error("token=history-maintenance-secret");
  };
  try {
