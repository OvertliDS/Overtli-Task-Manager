...
test("authoritative task transition matrix permits only documented lifecycle edges", () => {
  ...
  assert.throws(
    () => assertTaskTransition("pending", "done", { taskId: "task-1" }),
    (error) =>
      error.code === "INVALID_TRANSITION" &&
      error.details.from === "pending" &&
      error.details.to === "done",
  );
  assert.throws(() => assertTaskTransition("done", "dropped"), {
...
test("authoritative run transition matrix rejects reopening terminal history", () => {
  ...
  assert.throws(
    () => assertRunTransition("archived", "active", { runId: "run-1" }),
    { code: "INVALID_TRANSITION" },
  );
  assert.throws(() => assertRunTransition("completed", "active"), {
    code: "INVALID_TRANSITION",
  });
});
