  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      ["--input-type=module", "--eval", CHILD],
      {
        ...
        env: {
          ...env,
          OTM_TEST_WORKSPACE: input.args.workspaceRoot,
          OTM_TEST_INPUT: JSON.stringify(input),
        },
    ...
    child.on("exit", (code) => {
      ...
      try {
        ...
        reject(new Error(`invalid child result: ${stdout}`));
      }
    });
  });
}
...
async function race(packageRoot, env, input) {
  return Promise.all([
    concurrentChild(packageRoot, env, input),
    concurrentChild(packageRoot, env, input),
  ]);
}
...
function terminalizeInternalSteps(manager, workspaceRoot, taskId) {
  ...
  for (let index = 0; index < steps.length; index += 1) {
    manager.progress({
      workspaceRoot,
      taskId,
      internalStepIndex: index,
      internalStepStatus: "done",
      message: `Prepare concurrent terminal transition ${index}.`,
    });
  }
}
  ...
  for (const operation of ["progress", "reconcile", "complete", "clear"]) {
    ...
    if (operation === "reconcile")
      input = {
        ...
        args: {
          ...
          expectedRevision: 1,
          changes: [{ action: "add", title: "Concurrent addition" }],
    ...
    if (operation === "complete") {
      ...
      input = {
        ...
        args: {
          workspaceRoot,
          taskId,
          expectedRevision: manager.store.getRun(started.run.id).routeRevision,
          evidence: {
            kind: "test_result",
            summary: "Concurrent completion evidence.",
          },
        },
      };
    }
    if (operation === "clear") {
      ...
      manager.completeTask({
        workspaceRoot,
        taskId,
        evidence: { kind: "test_result", summary: "Prepare finalization." },
      });
      ...
      input = {
        ...
        args: {
          workspaceRoot,
          runId: started.run.id,
          expectedRevision: finalized.run.routeRevision,
        },
      };
    }
    ...
    assert.ok(
      ...
      `${operation} must produce one committed result: ${JSON.stringify(results)}`,
    );
    assert.ok(
      results.every(
        (result) =>
          ...
          result.code === "CLEAR_REQUIRES_FINALIZATION",
      ),
      `${operation} must fail safely: ${JSON.stringify(results)}`,
    );
    const run = manager.store.getRun(started.run.id);
    if (operation === "progress" || operation === "reconcile")
      assert.equal(run.routeRevision, 2);
