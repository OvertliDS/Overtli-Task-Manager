...
test('project review filters binary and oversized candidates before maxFiles', () => {
  ...
  fs.writeFileSync(path.join(docs, '01-large.md'), `# Large\n\n${'x'.repeat(400)}`, 'utf8');
  fs.writeFileSync(path.join(docs, '02-eligible.md'), '# Eligible\n\nThis must still be selected.\n', 'utf8');
  const review = reviewProjectContext({ workspaceRoot, maxFiles: 2, maxBytesPerFile: 256 });
  assert.equal(review.sourceCount, 2);
  assert.ok(review.sources.some((source) => source.path.replace(/\\/g, '/') === 'docs/02-eligible.md'));
...
test('project review enforces containment and reports limit omission deterministically', () => {
  ...
  const docs = path.join(workspaceRoot, 'docs');
  fs.mkdirSync(docs);
  for (let index = 0; index < 4; index += 1) fs.writeFileSync(path.join(docs, `guide-${index}.md`), `# Guide ${index}\n\nBody ${index}.\n`, 'utf8');
  const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'otm-review-outside-'));
  fs.writeFileSync(path.join(outside, 'secret.md'), '# Secret\n\nMUST_NOT_APPEAR\n', 'utf8');
  fs.symlinkSync(outside, path.join(docs, 'outside'), process.platform === 'win32' ? 'junction' : 'dir');
