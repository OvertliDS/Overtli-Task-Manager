...
export async function runMcpServer({ env = process.env } = {}) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
      ...
      const result = await dispatchTool({
        name,
        args,
        manager: name === "otm_doctor" ? null : getManager(),
        packageRoot,
        env,
      });
      ...
      return {
        ...
        content: [
          {
            type: "text",
            text: `OTM ${code}: ${error?.message || "Operation failed."}`,
          },
        ],
        structuredContent: {
          ok: false,
  ...
  server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => ({
    resourceTemplates: [
      {
        uriTemplate: "otm://workspace/{workspace}/session/{session}/current",
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (scoped) {
      const workspaceRoot = authorizeScopedResource(scoped, {
        env,
        cwd: process.cwd(),
      });
        ...
        contents: [
          {
            ...
            mimeType: scoped.markdown ? "text/markdown" : "application/json",
            text,
          },
        ],
      };
...
export function parseScopedResourceUri(uri) {
  const match =
    /^otm:\/\/workspace\/([^/]+)\/session\/([^/]+)\/(current(?:\.md)?)$/.exec(
      String(uri || ""),
    );
  ...
  try {
    ...
    if (
      !workspace ||
      !sessionId ||
      /%[0-9a-f]{2}/i.test(workspace) ||
      /[\\/]/.test(sessionId)
    )
      throw new OtmError("Invalid scoped OTM resource URI.", {
        code: "MCP_RESOURCE_URI_INVALID",
      });
    return { workspace, sessionId, markdown: match[3] === "current.md" };
  } catch {
    throw new OtmError("Invalid percent encoding in scoped OTM resource URI.", {
      code: "MCP_RESOURCE_URI_INVALID",
    });
  }
}
...
export function authorizeScopedResource(
  ...
  const denied = () => {
    throw new OtmError("Scoped OTM resource access is not authorized.", {
      code: "MCP_RESOURCE_SCOPE_DENIED",
    });
  };
  const currentWorkspace = realCanonicalWorkspace(
    findWorkspaceRoot(cwd),
    denied,
  );
  const callerSession = resolveSessionId({}, env);
  if (!callerSession || scoped.sessionId !== callerSession) denied();
  if (!isAbsolute(scoped.workspace)) denied();
  const requested = realCanonicalWorkspace(scoped.workspace, denied);
  const configured = String(env.OTM_ALLOWED_WORKSPACES || "")
    .split(";")
    .map((value) => value.trim())
    .filter(Boolean)
    .map((value) => realCanonicalWorkspace(value, denied));
  const allowed = [currentWorkspace, ...configured];
  if (!allowed.some((root) => sameRealPath(root, requested))) denied();
  return requested;
}

function realCanonicalWorkspace(value, denied) {
  try {
    return fs.realpathSync.native(canonicalizeWorkspaceRoot(value).displayPath);
  } catch {
    denied();
  }
}
...
export function validateMcpArgs(name, args) {
  ...
  if (!definition)
    throw new OtmError(`Unknown tool: ${name}`, { code: "MCP_TOOL_UNKNOWN" });
  if (!args || typeof args !== "object" || Array.isArray(args))
    throw new OtmError("MCP tool arguments must be an object.", {
  ...
  if (unknown.length)
    throw new OtmError("MCP arguments include unsupported properties.", {
  ...
  for (const [key, value] of Object.entries(args)) {
    if (typeof value === "string" && value.length > 16_000)
      throw sizeError(`arguments.${key}`, 16_000, "string");
    if (Array.isArray(value) && value.length > 256)
      throw sizeError(`arguments.${key}`, 256, "array");
  }
...
function validateSchemaValue(value, schema = {}, field) {
  ...
  if (Array.isArray(schema.oneOf)) {
    const matches = schema.oneOf.filter((candidate) => {
      try {
        validateSchemaValue(value, candidate, field);
        return true;
      } catch {
        return false;
      }
  ...
  if (schema.type === "object") {
    ...
    for (const required of schema.required || [])
      if (value[required] === undefined)
        throw schemaError(`${field}.${required}`, "is required");
    if (schema.additionalProperties === false) {
      ...
      if (unknown.length)
        throw new OtmError(
          "MCP nested arguments include unsupported properties.",
          {
            code: "MCP_UNKNOWN_ARGUMENT",
            details: { fields: unknown.map((key) => `${field}.${key}`) },
          },
        );
    }
    for (const [key, child] of Object.entries(value))
      if (Object.hasOwn(properties, key))
        validateSchemaValue(child, properties[key], `${field}.${key}`);
    return;
  }
  if (schema.type === "array") {
    if (!Array.isArray(value)) throw schemaError(field, "must be an array");
    if (schema.maxItems !== undefined && value.length > schema.maxItems)
      throw sizeError(field, schema.maxItems, "array");
    for (const [index, item] of value.entries())
      validateSchemaValue(item, schema.items || {}, `${field}[${index}]`);
    return;
  }
  if (schema.type === "string") {
    ...
    if (schema.maxLength !== undefined && value.length > schema.maxLength)
      throw sizeError(field, schema.maxLength, "string");
    ...
    throw schemaError(field, "must be an integer in range");
  else if (
    schema.type === "number" &&
    ...
    throw schemaError(field, "must be a number in range");
  if (Array.isArray(schema.enum) && !schema.enum.includes(value))
    throw schemaError(field, "has an unsupported value");
}

function schemaError(field, reason) {
  return new OtmError(`MCP argument ${field} ${reason}.`, {
    code: "MCP_INVALID_ARGUMENTS",
    details: { field, reason },
  });
}
function sizeError(field, maximum, type) {
  return new OtmError(`MCP ${type} argument exceeds the maximum size.`, {
    code: "INPUT_TOO_LARGE",
    details: { field, maximum },
  });
}
...
async function dispatchTool({ name, args, manager, packageRoot, env }) {
  switch (name) {
    ...
    case "otm_reconcile":
      return manager.reconcile(args);
    case "otm_snapshot":
      return manager.snapshot({ ...args, write: false });
    case "otm_start_task":
      return manager.markTaskActive(args);
    case "otm_progress":
      return manager.progress(args);
    case "otm_complete_task":
      return manager.completeTask(args);
    case "otm_block_task":
      return manager.blockTask(args);
    case "otm_drop_task":
    ...
    case "otm_clear_current":
      return manager.clearCurrent(args);
    case "otm_abandon":
      return manager.abandonRun(args);
    case "otm_resume":
      return manager.resumeRun(args);
    case "otm_archive":
      return manager.archiveRun(args);
    case "otm_cleanup_workspace":
      return manager.cleanupWorkspace(args);
    case "otm_prune_history":
      return manager.pruneHistory(args);
    case "otm_memory_search":
      return memorySearchResult(manager.searchMemory(args));
    case "otm_memory_upsert":
      return {
        ...manager.upsertMemory(args),
        markdown: `## ✅ OTM memory updated\n\nSaved: **${args.title}**\n`,
      };
    case "otm_memory_delete":
      return {
        ...manager.deleteMemory(args),
        markdown:
          "## ✅ OTM memory cleanup\n\nRequested memory entries were removed when matched.\n",
      };
    case "otm_memory_list":
      return memorySearchResult(manager.listMemory(args));
    ...
    case "otm_memory_purge_expired":
      return {
        ...manager.purgeExpiredMemory(args),
        markdown:
    ...
    case "otm_doctor": {
      const workspaceRoot =
        args.workspaceRoot || findWorkspaceRoot(process.cwd());
...
function memorySearchResult(result) {
  ...
  if (!result.entries.length)
    lines.push("No matching project memory entries found.");
  for (const entry of result.entries) {
    lines.push(`### ${entry.title}`);
    lines.push(`Kind: ${entry.kind} · Score: ${entry.score}`);
    lines.push("");
    lines.push(
      entry.body.length > 1200 ? `${entry.body.slice(0, 1200)}\n…` : entry.body,
    );
    lines.push("");
  }
