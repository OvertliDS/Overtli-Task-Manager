...
import { canonicalizeWorkspaceRoot, resolveWithinRoot } from '../core/validation.mjs';

const ROOT_FILES = [
  'AGENTS.md', 'AGENTS.override.md',
  'README.md', 'readme.md', 'ARCHITECTURE.md', 'architecture.md', 'PRODUCT.md', 'PRD.md', 'GDD.md', 'GAME_DESIGN_DOCUMENT.md', 'CONTRIBUTING.md', 'CHANGELOG.md',
...
export function reviewProjectContext({ cwd = process.cwd(), workspaceRoot = null, maxFiles = 20, maxBytesPerFile = 12000 } = {}) {
  const root = canonicalizeWorkspaceRoot(workspaceRoot || findWorkspaceRoot(cwd)).displayPath;
  if (!Number.isInteger(maxFiles) || maxFiles < 1 || maxFiles > 100) throw new Error('maxFiles must be an integer between 1 and 100.');
  if (!Number.isInteger(maxBytesPerFile) || maxBytesPerFile < 256 || maxBytesPerFile > 256 * 1024) throw new Error('maxBytesPerFile must be an integer between 256 and 262144.');
  const candidateResult = collectCandidateFilesDetailed(root);
  const skipped = { ...candidateResult.skipped, limit: 0, oversized: 0, binary: 0, empty: 0 };
  ...
  for (const filePath of candidateResult.files) {
    ...
    if (!st || !st.isFile()) { skipped.notRegular = (skipped.notRegular || 0) + 1; continue; }
    if (st.size > maxBytesPerFile) { skipped.oversized += 1; continue; }
    if (isBinaryFile(filePath)) { skipped.binary += 1; continue; }
  ...
  for (const { filePath, stat: st } of candidates) {
    ...
    if (!text.trim()) { skipped.empty += 1; continue; }
  ...
  const diagnostics = { algorithmVersion: 'otm.project-review.v2', candidateCount: eligible.length, readCount: sources.length, skipped, limitsOmittedFiles: skipped.limit > 0 };
  const fingerprint = shortHash(JSON.stringify({ sources: sources.map((src) => [src.path, src.hash]), maxFiles, maxBytesPerFile, algorithmVersion: diagnostics.algorithmVersion }));
  const reviewJsonPath = path.join(cacheDir(root), 'project-review.json');
  const existing = readJson(reviewJsonPath, null);
  if (existing?.schemaVersion === 'otm.project-review.v2' && existing.fingerprint === fingerprint) {
    return {
      ...existing,
...
function collectCandidateFilesDetailed(root) {
  ...
  const seen = new Set();
  const out = [];
  const skipped = { outsideWorkspace: 0, notRegular: 0, unsupportedExtension: 0, duplicate: 0 };
  function add(filePath) {
    let resolved = path.resolve(filePath);
    try {
      resolved = fs.realpathSync.native(resolved);
    } catch { skipped.notRegular += 1; return false; }
    try { resolveWithinRoot(canonicalRoot, resolved, { allowAbsolute: true, mustExist: true }); } catch { skipped.outsideWorkspace += 1; return false; }
    const stat = statSafe(resolved);
    if (!stat?.isFile()) { skipped.notRegular += 1; return false; }
    if (seen.has(resolved)) { skipped.duplicate += 1; return false; }
    const ext = path.extname(resolved).toLowerCase();
    if (!DOC_EXTENSIONS.has(ext)) { skipped.unsupportedExtension += 1; return false; }
    const rel = relativeToWorkspace(canonicalRoot, resolved);
    if (rel.startsWith('..') || rel.includes('node_modules/') || rel.includes('.git/') || rel.includes('dist/') || rel.includes('build/')) { skipped.outsideWorkspace += 1; return false; }
    seen.add(resolved);
    out.push(resolved);
    return true;
  }
...
function renderProjectSummary(root, sources, sections, diagnostics) {
  ...
  lines.push(`Skipped: ${Object.values(diagnostics.skipped).reduce((total, value) => total + Number(value || 0), 0)}${diagnostics.limitsOmittedFiles ? ' (file limit omitted eligible files)' : ''}`);
  lines.push('');
