...
export class JsonStore {
  ...
  #read() {
    ...
    try {
      ...
      if (!data || typeof data !== 'object' || Array.isArray(data)) throw new OtmError('JSON store document is invalid.', { code: 'JSON_STORE_CORRUPTION' });
      const normalized = {
      ...
      if (error?.code === 'JSON_STORE_CORRUPTION') throw this.#quarantine(error);
  ...
  #quarantine(cause) {
    ...
    let preserved = false;
    try { fs.renameSync(this.filePath, quarantinePath); preserved = true; } catch {}
    return new OtmError('JSON store is corrupted and was quarantined. Restore a backup or repair the preserved state before retrying.', {
      code: 'JSON_STORE_CORRUPTION',
      details: { quarantinePath: preserved ? quarantinePath : null, preserved, recovery: `${this.filePath}.backup` }
    });
  }

  #write(data) {
    if (this.readOnly) throw new OtmError('Store was opened read-only.', { code: 'STORE_READ_ONLY' });
    ...
    if (fs.existsSync(this.filePath)) {
      ...
      try { fs.copyFileSync(this.filePath, backupPath); } catch (error) {
        throw new OtmError('Unable to create JSON store recovery backup.', { code: 'JSON_STORE_BACKUP_FAILED', details: { backupPath } });
      }
    }
  ...
  #withLock(fn) {
    ...
    while (true) {
      ...
      try {
        ...
        fs.writeFileSync(handle, JSON.stringify({ pid: process.pid, startedAt: new Date().toISOString(), operation: 'store-transaction', heartbeatAt: new Date().toISOString() }), 'utf8');
        return fn();
      } catch (error) {
        ...
        if (error?.code !== 'EEXIST' && !(error?.code === 'EPERM' && fs.existsSync(this.lockPath))) throw error;
  ...
  createRoute({ run, tasks, event, replaceRunId = null }) {
    return this.transaction((data) => {
      const active = data.runs.find((item) => item.workspaceRoot === run.workspaceRoot
        && (item.sessionId || null) === (run.sessionId || null)
        && ['active', 'ready_to_finalize', 'blocked', 'paused'].includes(item.status));
      if (active && active.id !== replaceRunId) {
        throw new OtmError('An active route already exists for this workspace and session.', { code: 'ACTIVE_ROUTE_CONFLICT', details: { runId: active.id, routeRevision: active.routeRevision } });
      }
      if (replaceRunId) {
        const prior = data.runs.find((item) => item.id === replaceRunId);
        if (!prior) throw new OtmError('Route selected for replacement no longer exists.', { code: 'RUN_NOT_FOUND' });
        prior.status = 'abandoned';
        prior.finalizedAt = run.createdAt;
        prior.updatedAt = run.createdAt;
        prior.metadata = { ...(prior.metadata || {}), abandonedReason: 'Replaced by new route' };
      }
      data.runs.push(run);
      data.tasks.push(...tasks);
      if (!data.events.some((item) => item.idempotencyKey === event.idempotencyKey)) data.events.push(event);
      return run;
    });
  }
  ...
  commitRunMutation({ run, expectedRevision, tasks = [], newTasks = [], summaries = [], event = null }) {
    return this.transaction((data) => {
      ...
      if (runIndex < 0) throw new OtmError('Run not found.', { code: 'RUN_NOT_FOUND' });
      ...
      for (const task of tasks) {
        const index = data.tasks.findIndex((item) => item.id === task.id && item.runId === run.id);
        if (index < 0) throw new OtmError('Task not found in run.', { code: 'TASK_NOT_FOUND', details: { taskId: task.id, runId: run.id } });
        data.tasks[index] = { ...task, updatedAt: task.updatedAt || nowIso() };
      }
      for (const task of newTasks) {
        if (task.runId !== run.id || data.tasks.some((item) => item.id === task.id)) {
      ...
      if (event && !data.events.some((item) => item.idempotencyKey === event.idempotencyKey)) data.events.push(event);
  ...
  listActiveRuns(workspaceRoot) {
    return this.#read().runs
      .filter((run) => run.workspaceRoot === workspaceRoot && ['active', 'ready_to_finalize', 'blocked', 'paused'].includes(run.status))
      .sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt)));
  }
  ...
  claimLegacyActiveRun(workspaceRoot, sessionId, metadata = {}) {
    return this.transaction((data) => {
      const run = data.runs
        .filter((item) => item.workspaceRoot === workspaceRoot && !item.sessionId && ['active', 'ready_to_finalize', 'blocked', 'paused'].includes(item.status))
  ...
  exportWorkspace(workspaceRoot) {
    ...
    return {
      ...
      cache: data.cache.filter((entry) => entry.workspaceRoot === workspaceRoot)
    };
  }
  ...
  upsertCache(entry) {
    return this.transaction((data) => {
      ...
      const next = { ...entry, createdAt: index >= 0 ? data.cache[index].createdAt : entry.createdAt };
  ...
  deleteCache(filter = {}) {
    if (!filter.id && !filter.workspaceRoot && !filter.kind && !filter.tag && !filter.expired) {
      throw new OtmError('Cache deletion requires at least one selector.', { code: 'CACHE_SELECTOR_REQUIRED' });
    }
    return this.transaction((data) => {
      ...
      data.cache = data.cache.filter((entry) => {
        ...
        if (filter.tag && !(entry.tags || []).map((tag) => String(tag).trim().toLowerCase()).includes(String(filter.tag).trim().toLowerCase())) return true;
        if (filter.expired && (!entry.expiresAt || String(entry.expiresAt) > String(filter.now || nowIso()))) return true;
        return false;
      });
...
function validateStateDocument(data) {
  ...
  for (const name of collections) {
    if (!Array.isArray(data[name])) throw new OtmError('JSON store document has an invalid collection.', { code: 'JSON_STORE_CORRUPTION', details: { collection: name } });
    const ids = new Set();
    for (const item of data[name]) {
      if (!item || typeof item !== 'object' || typeof item.id !== 'string' || !item.id) {
        throw new OtmError('JSON store document has an invalid record.', { code: 'JSON_STORE_CORRUPTION', details: { collection: name } });
      }
      if (ids.has(item.id)) throw new OtmError('JSON store document contains duplicate identifiers.', { code: 'JSON_STORE_CORRUPTION', details: { collection: name, id: item.id } });
      ids.add(item.id);
    }
  }
  const runIds = new Set(data.runs.map((run) => run.id));
  for (const [name, records] of [['tasks', data.tasks], ['events', data.events], ['summaries', data.summaries]]) {
    for (const record of records) {
      if (!runIds.has(record.runId)) throw new OtmError('JSON store document has an orphaned record.', { code: 'JSON_STORE_CORRUPTION', details: { collection: name, id: record.id, runId: record.runId } });
    }
  }
}
...
function assertImportDoesNotConflict(data, payload) {
  for (const name of ['runs', 'tasks', 'events', 'summaries', 'cache']) {
    ...
    if (duplicate) throw new OtmError('Historical import conflicts with an existing record.', { code: 'IMPORT_CONFLICT', details: { collection: name, id: duplicate.id } });
  }
  const currentEventKeys = new Set(data.events.map((event) => event.idempotencyKey));
  const duplicateEvent = payload.events.find((event) => currentEventKeys.has(event.idempotencyKey));
  if (duplicateEvent) throw new OtmError('Historical import conflicts with an existing event idempotency key.', { code: 'IMPORT_CONFLICT', details: { collection: 'events', idempotencyKey: duplicateEvent.idempotencyKey } });
}
