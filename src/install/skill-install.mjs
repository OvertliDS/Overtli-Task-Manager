...
export function installRepoSkills({ workspaceRoot, packageRoot, targetRoot = null, dryRun = false } = {}) {
  ...
  for (const skillName of fs.readdirSync(source)) {
    ...
    installed.push({ name: skillName, path: targetSkill, directory: targetSkillDir, files: targetFiles, changed });
  }
...
function listSkillFiles(root) {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...listSkillFiles(fullPath).map((child) => path.join(entry.name, child)));
