...
/** @param {any} options */
export function patchGitignore(options = {}) {
  const { workspaceRoot, dryRun = false } = options;
  const filePath = path.join(workspaceRoot, ".gitignore");
  const before = readText(filePath, "");
  if (
    markerCount(before, BLOCK_BEGIN) > 1 ||
    markerCount(before, BLOCK_END) > 1
  ) {
    return {
      ok: false,
      action: "conflict",
      filePath,
      reason: "Gitignore contains duplicate OTM managed block markers.",
    };
  }
...
${BLOCK_END}`;
  ...
  if (begin >= 0 && end >= 0 && end > begin) {
    ...
    return {
      ...
      action: "conflict",
      filePath,
      reason: "Gitignore managed block markers are incomplete.",
    };
  } else {
  ...
  return {
    ...
    filePath,
    dryRun,
    action: after === before ? "unchanged" : "updated",
