...
/** @param {any} options */
export function uninstallWorkspace(options = {}) {
  const {
    cwd = process.cwd(),
    workspaceRoot = null,
    packageRoot,
      ...
      results: [
        {
          ...
          reason:
            "Uninstall requires --confirm after reviewing a dry-run preview.",
        },
      ],
    };
  ...
  const originals = snapshotPaths(
    preflight.flatMap((item) => item.affectedPaths || []).filter(Boolean),
  );
  ...
  try {
    ...
    const manifest = {
      schemaVersion: "otm.uninstall.v1",
      workspaceRoot: root,
      packageRoot,
      removeState,
      transactionId,
...
function buildPlans({ root, packageRoot, removeState, targetAgentsFile }) {
  const agents = targetAgentsFile
    ? resolveWithinRoot(root, targetAgentsFile)
...
function markerPlan(step, filePath, beginMarker, endMarker) {
  const build = () =>
    removeMarkedBlock(readText(filePath, ""), beginMarker, endMarker);
  return {
    step,
    preview() {
      const result = build();
  ...
  const build = () => {
    ...
    try {
      ...
      return {
        ok: false,
        action: "invalid-json",
        reason: "hooks.json is malformed. Uninstall was not applied.",
      };
    }
    try {
...
function skillPlan(root, packageRoot) {
  ...
  const preview = () => {
    ...
    for (const entry of fs.readdirSync(sourceRoot, { withFileTypes: true })) {
      ...
      if (matchesOwnedSkill(sourceDir, targetDir))
        results.push({
          name: entry.name,
          path: targetDir,
          action: "removed",
    ...
    return {
      ...
      installed: results,
      affectedPaths: removable.map((item) => item.path),
    };
  };
  return {
    step: "skills",
    preview: () => ({ step: "skills", ...preview(), dryRun: true }),
    apply: () => {
...
function statePlan(root) {
  ...
  const build = () => {
    ...
    if (inspection.error)
      return { ok: false, action: "corrupt-state", reason: inspection.error };
    if (inspection.activePaths.length)
      return {
        ok: false,
        action: "active-state",
        reason: `Refusing to remove state while ${inspection.activePaths.length} active route snapshot(s) exist. Finalize and clear, or explicitly abandon the routes first.`,
        activePaths: inspection.activePaths,
      };
...
function inspectCurrentFiles(stateDir) {
  ...
  if (fs.existsSync(sessionsRoot)) {
    for (const entry of fs.readdirSync(sessionsRoot, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && fs.existsSync(current))
  ...
  for (const current of currentFiles) {
    let snapshot;
    try {
      snapshot = JSON.parse(fs.readFileSync(current, "utf8"));
    } catch {
      return {
        activePaths,
        error: `Current state snapshot is malformed: ${current}. Preserve it and repair before removing state.`,
      };
    }
    if (
...
function removeMarkedBlock(before, beginMarker, endMarker) {
  ...
  if (beginCount > 1 || endCount > 1 || beginCount !== endCount)
    return {
      ...
      action: "conflict",
      reason:
        "Managed markers are duplicate or incomplete. Manual repair is required before uninstall.",
    };
  if (!beginCount)
    return { ok: true, action: "unchanged", changed: false, after: before };
  ...
  if (end < begin)
    return {
      ...
      action: "conflict",
      reason:
        "Managed markers are out of order. Manual repair is required before uninstall.",
    };
  const after =
    `${before.slice(0, begin).trimEnd()}\n${before.slice(end + endMarker.length).trimStart()}`.trimEnd() +
    (before.trim() ? "\n" : "");
...
function matchesOwnedSkill(sourceDir, targetDir) {
  ...
  if (
    relativeFiles.length !== targetFiles.length ||
    relativeFiles.some((file, index) => file !== targetFiles[index])
  )
    return false;
  return relativeFiles.every(
    (relative) =>
      hashFile(path.join(sourceDir, relative)) ===
      hashFile(path.join(targetDir, relative)),
  );
}
...
function listFiles(root) {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory())
      files.push(
        ...listFiles(full).map((child) => path.join(entry.name, child)),
      );
...
function writeBackups(backupPath, root, originals) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ schemaVersion: "otm.uninstall-backup.v1", workspaceRoot: root, files }, null, 2)}\n`,
    "utf8",
  );
}
...
function restorePaths(originals, root) {
  for (const item of originals) {
    if (item.isFile) {
      ...
      if (backupRoot)
        fs.cpSync(path.join(backupRoot, `${index}.dir`), item.filePath, {
          recursive: true,
          force: true,
        });
    } else if (!item.exists) {
...
export function renderUninstallResult(result) {
  const lines = [
    "## Overtli Task Manager uninstall",
    "",
  ...
  lines.push(
    "",
    "Workspace state and summaries are retained unless --remove-state is explicitly confirmed.",
  );
  return `${lines.join("\n")}\n`;
}
