...
import { findWorkspaceRoot, ensureDir, workspaceStateDir, workspaceTempDir, atomicWriteJson, readJson, hashFile } from '../core/fs-utils.mjs';
...
export function installWorkspace({ cwd = process.cwd(), workspaceRoot = null, packageRoot, dryRun = false, installMcpConfig = false, targetAgentsFile = null } = {}) {
  ...
  const steps = () => [
    { step: 'agents', run: (preview) => patchAgentsFile({ workspaceRoot: root, targetFile: targetAgentsFile, dryRun: preview }) },
    { step: 'hooks', run: (preview) => patchHooksJson({ workspaceRoot: root, packageRoot, dryRun: preview }) },
    { step: 'skills', run: (preview) => installRepoSkills({ workspaceRoot: root, packageRoot, dryRun: preview }) },
    { step: 'gitignore', run: (preview) => patchGitignore({ workspaceRoot: root, dryRun: preview }) },
    ...(installMcpConfig ? [{ step: 'mcp-config', run: (preview) => patchProjectMcpConfig({ workspaceRoot: root, packageRoot, dryRun: preview }) }] : [])
  ];
  ...
  const preflight = steps().map((item) => ({ step: item.step, ...item.run(true) }));
  if (dryRun || preflight.some((item) => item.ok === false)) {
    const manifest = buildInstallManifest({ root, packageRoot, dryRun: true, results: preflight });
    return { ok: preflight.every((r) => r.ok !== false), workspaceRoot: root, dryRun: true, manifest, results: preflight };
  }
  ...
  const backupPath = path.join(workspaceStateDir(root), 'install-backups', transactionId);
  try {
    ...
    for (const item of steps()) {
      ...
      results.push(result);
      if (result.ok === false) throw new Error(`${item.step}: ${result.reason || result.action || 'installation failed'}`);
    }
    const manifest = buildInstallManifest({ root, packageRoot, dryRun, results, transactionId, backupPath, before: originals });
    atomicWriteJson(path.join(workspaceStateDir(root), 'install.json'), manifest, { tempDir: workspaceTempDir(root) });
    return { ok: true, workspaceRoot: root, dryRun, manifest, backupPath, results };
  } catch (error) {
    ...
    return {
      ...
      rolledBack: true,
      backupPath,
      results: [{ step: 'transaction', ok: false, action: 'rolled-back', reason: String(error.message || error) }]
    };
  }
}
...
function managedPaths(results) {
  return [...new Set(results.flatMap((item) => [
    item.filePath,
    ...(Array.isArray(item.installed) ? item.installed.flatMap((skill) => skill.files || [skill.path]) : [])
...
function snapshotFiles(paths) {
  return paths.map((filePath) => {
    ...
    return { filePath, existed, isFile, content: isFile ? fs.readFileSync(filePath) : null, mode: isFile ? stat.mode : null, beforeHash: isFile ? hashFile(filePath) : null };
  });
}

function writeBackups(backupPath, root, originals) {
  const manifest = originals.map((item, index) => ({ filePath: item.filePath, existed: item.existed, isFile: item.isFile, beforeHash: item.beforeHash, backupFile: item.isFile ? `${index}.bak` : null }));
  ...
  fs.writeFileSync(path.join(backupPath, 'manifest.json'), `${JSON.stringify({ schemaVersion: 'otm.install-backup.v1', workspaceRoot: root, files: manifest }, null, 2)}\n`, 'utf8');
}
...
function buildInstallManifest({ root, packageRoot, dryRun, results, transactionId = null, backupPath = null, before = [] }) {
  ...
  return {
    ...
    steps: results.map((item) => ({
      ...
      beforeHash: before.find((file) => file.filePath === item.filePath)?.beforeHash || null,
      afterHash: item.filePath ? hashFile(item.filePath) : null,
      ownedFiles: Array.isArray(item.installed) ? item.installed.flatMap((skill) => (skill.files || [skill.path]).map((filePath) => ({
        skill: skill.name,
        path: filePath,
