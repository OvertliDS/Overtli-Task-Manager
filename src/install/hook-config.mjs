...
export function patchHooksJson({ workspaceRoot, packageRoot, targetFile = null, dryRun = false } = {}) {
  ...
  try {
    ...
    return { ok: false, action: 'invalid-json', filePath, reason: 'hooks.json is malformed. Installation was not applied.', errorCode: 'HOOKS_JSON_INVALID' };
  }
  if (!doc || typeof doc !== 'object' || Array.isArray(doc) || (doc.hooks !== undefined && (!doc.hooks || typeof doc.hooks !== 'object' || Array.isArray(doc.hooks)))) {
    return { ok: false, action: 'invalid-json', filePath, reason: 'hooks.json must contain an object-valued hooks property. Installation was not applied.', errorCode: 'HOOKS_JSON_INVALID' };
  }
...
export function isOtmHookCommand(command) {
  ...
  return /^(?:node(?:\.exe)?|["'][^"']*node(?:\.exe)?["'])\s+(?:"[^"]*[\\/]bin[\\/]otm\.mjs"|[^\s]*[\\/]bin[\\/]otm\.mjs)\s+hook\s+[a-z][a-z-]*(?:\s.*)?$/i.test(command.trim());
}

export function removeOtmHooksDocument(doc) {
  if (!doc || typeof doc !== 'object' || Array.isArray(doc) || !doc.hooks || typeof doc.hooks !== 'object' || Array.isArray(doc.hooks)) {
