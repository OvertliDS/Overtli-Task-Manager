...
import { findWorkspaceRoot, readText, getHomeDir, atomicWriteJson } from '../core/fs-utils.mjs';
import { installWorkspace, renderInstallResult } from '../install/install-workspace.mjs';
import { uninstallWorkspace, renderUninstallResult } from '../install/uninstall-workspace.mjs';
import { installGlobal, renderGlobalInstallResult } from '../install/install-global.mjs';
import { uninstallGlobal, renderGlobalUninstallResult } from '../install/uninstall-global.mjs';
...
export async function handleCli({ argv, cwd, stdin, packageRoot, env }) {
  ...
  const flags = parseFlags(command === 'hook' ? argv.slice(2) : argv.slice(1), allowedFlagsFor(command), booleanFlagsFor(command));
  validateNumericFlags(flags);
  if (command === 'backup' || command === 'restore') {
    const result = command === 'backup' ? await backupStore(env, flags) : restoreStore(env, flags);
    printCli(flags, result, `OTM ${command}\nStorage: ${result.storage}\n${result.dryRun ? 'Mutations: none' : `Path: ${result.path}`}`);
    return;
  }
  if (command === 'migrate' && flags.dryRun) {
    const result = inspectMigrationDryRun(env);
    console.log(flags.json ? JSON.stringify(result, null, 2) : `OTM migration dry run\nStorage: ${result.storage}\nCurrent schema: ${result.currentVersion ?? 'n/a'}\nTarget schema: ${result.targetVersion ?? 'n/a'}\nMutations: none`);
    return;
  }
  const hookEventName = command === 'hook' ? argv[1] : null;
  if (command === 'hook' && (!hookEventName || hookEventName.startsWith('--'))) throw new Error('hook requires an event name.');
  ...
  if (command === 'doctor') {
    ...
    if (flags.repair === true) {
      if (!report.ok) {
        report.repair = { attempted: false, reason: 'Doctor found integrity errors; restore or resolve them before summary repair.' };
      } else {
        const manager = createTaskManager({ cwd: workspaceRoot, env, readOnly: flags.dryRun === true });
        try {
          report.repair = { attempted: true, ...manager.repairSummaries({ workspaceRoot, dryRun: flags.dryRun === true }) };
  ...
  const readOnly = flags.dryRun === true && new Set(['repair', 'export', 'import', 'memory-purge-expired', 'prune-history', 'cleanup']).has(command);
  const manager = createTaskManager({ cwd: workspaceRoot, env, readOnly });

  if (command === 'migrate') {
    const result = { storage: manager.store.kind, schemaVersion: manager.store.kind === 'sqlite' ? manager.store.db.pragma('user_version', { simple: true }) : 'otm.store.v1', migrated: true };
    printCli(flags, result, `OTM migration complete\nStorage: ${result.storage}\nSchema: ${result.schemaVersion}`);
    return;
  }

  if (command === 'repair') {
    const result = manager.repairSummaries({ workspaceRoot, dryRun: flags.dryRun === true });
    printCli(flags, result, `OTM repair\nSummary records: ${result.matched}\nFiles republished: ${result.repaired}${result.dryRun ? '\nMutations: none' : ''}`);
    return;
  }
  ...
  if (command === 'export') {
    ...
    const result = { workspaceRoot, output, dryRun: flags.dryRun === true, counts: exportCounts(document) };
    if (!result.dryRun) {
      if (fs.existsSync(output)) throw new Error('export output already exists; choose a new path to avoid overwriting a backup.');
      atomicWriteJson(output, document, { tempDir: path.dirname(output) });
    }
    printCli(flags, result, `OTM export\nWorkspace: ${workspaceRoot}\nRuns: ${result.counts.runs}\nOutput: ${output}${result.dryRun ? '\nMutations: none' : ''}`);
    return;
  }

  if (command === 'import') {
    if (flags.confirm !== true) throw new Error('import requires --confirm after reviewing the export file.');
    if (!flags.input) throw new Error('import requires --input.');
    const input = path.resolve(flags.input);
    if (!fs.existsSync(input) || !fs.statSync(input).isFile()) throw new Error('import input must be an existing export file.');
    let document;
    try { document = JSON.parse(fs.readFileSync(input, 'utf8')); } catch { throw new Error('import input is malformed JSON.'); }
    const result = manager.importHistorical({ workspaceRoot, document, dryRun: flags.dryRun === true });
    printCli(flags, { ...result, input }, `OTM historical import\nWorkspace: ${workspaceRoot}\nRuns: ${result.imported.runs}${result.dryRun ? '\nMutations: none' : ''}`);
    return;
  }

  if (command === 'resume') {
    if (!flags.runId) throw new Error('resume requires --run-id.');
    const result = manager.resumeRun({ workspaceRoot, sessionId, runId: flags.runId, taskId: flags.taskId, reason: flags.reason });
    printCli(flags, { run: result.run, snapshot: result.snapshot }, result.markdown);
    return;
  }

  if (command === 'archive') {
    if (!flags.runId) throw new Error('archive requires --run-id.');
    if (flags.confirm !== true) throw new Error('archive requires --confirm after reviewing the run id.');
    const result = manager.archiveRun({ workspaceRoot, sessionId, runId: flags.runId, reason: flags.reason });
    printCli(flags, result, `OTM archive\nRun: ${result.run.id}\n${result.idempotent ? 'Already archived.' : 'Archived.'}`);
    return;
  }
  ...
  if (command === 'abandon') {
    ...
    if (flags.confirm !== true) throw new Error('abandon requires --confirm after reviewing the run id and reason.');
    const result = manager.abandonRun({ workspaceRoot, sessionId, runId: flags.runId, reason: flags.reason, deleteFiles: flags.deleteFiles === true });
  ...
  if (command === 'list-runs') {
    const runs = manager.listRuns({ workspaceRoot, limit: flags.limit === undefined ? 20 : Number(flags.limit) });
    const markdown = ['## OTM runs', '', ...runs.runs.map((run) => `- ${run.id} — ${run.status} — ${run.goal}`), ''].join('\n');
  ...
  if (command === 'history') {
    ...
    if (!run || run.workspaceRoot !== workspaceRoot) throw new Error('Run not found in this workspace.');
    const events = manager.store.getEvents(run.id, flags.limit === undefined ? 100 : Number(flags.limit));
    const markdown = ['## OTM history', '', ...events.map((event) => `- ${event.createdAt} — ${event.eventType}`), ''].join('\n');
    printCli(flags, { runId: run.id, events }, markdown);
    return;
  }

  if (command === 'memory-list') {
    const result = manager.listMemory({ workspaceRoot, limit: flags.limit === undefined ? 50 : Number(flags.limit) });
  ...
  if (command === 'memory-inspect') {
    ...
    printCli(flags, result, `## OTM memory\n\n${result.entry.title}\n\n${result.entry.body}\n`);
    return;
  }

  if (command === 'memory-purge-expired') {
    const result = manager.purgeExpiredMemory({ workspaceRoot, dryRun: flags.dryRun === true });
    printCli(flags, result, `## OTM expired memory purge\n\nMatched: ${result.matched}\nDeleted: ${result.deleted}\n`);
    return;
  }
...
function parseFlags(items, allowed, booleanFlags = new Set()) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    if (!allowed.has(key)) throw new Error(`Unknown flag for this command: --${rawKey}`);
    if (Object.hasOwn(flags, key)) throw new Error(`Duplicate flag: --${rawKey}`);
    if (inlineValue !== undefined) {
      if (booleanFlags.has(key)) {
        if (inlineValue === '') { flags[key] = true; continue; }
        if (inlineValue !== 'true' && inlineValue !== 'false') throw new Error(`Boolean flag --${rawKey} must be true or false.`);
        flags[key] = inlineValue === 'true';
        continue;
      }
      if (inlineValue === '') throw new Error(`Flag --${rawKey} requires a value.`);
      flags[key] = inlineValue;
      continue;
    }
    const next = items[i + 1];
    if (!next || next.startsWith('--')) {
      if (!booleanFlags.has(key)) throw new Error(`Flag --${rawKey} requires a value.`);
      flags[key] = true;
    } else if (booleanFlags.has(key)) {
      if (next === 'true' || next === 'false') { flags[key] = next === 'true'; i += 1; }
      else flags[key] = true;
...
function booleanFlagsFor(command) {
  ...
  const byCommand = {
    ...
    'install-global': ['dryRun'], uninstall: ['dryRun', 'confirm', 'removeState', 'global'],
    doctor: ['repair', 'dryRun'], migrate: ['dryRun'], repair: ['dryRun'], export: ['dryRun'], import: ['confirm', 'dryRun'], resume: [], archive: ['confirm'], abandon: ['confirm', 'deleteFiles'],
    'memory-purge-expired': ['dryRun'],
    'clear-current': ['deleteFiles'],
    cleanup: ['allSessions', 'confirm', 'dryRun'],
    'prune-history': ['dryRun'], backup: ['dryRun'], restore: ['confirm', 'dryRun']
  };
...
function validateNumericFlags(flags) {
  ...
  for (const key of positiveIntegers) {
    ...
    if (!Number.isInteger(value) || value < 1 || value > (key === 'maxFiles' ? 100 : 10_000)) {
      throw new Error(`--${key.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)} must be an integer in range.`);
    }
    flags[key] = value;
  }
  for (const key of ['minAgeMs', 'scratchMaxAgeMs', 'retentionDays']) {
    if (flags[key] === undefined) continue;
    const value = Number(flags[key]);
    if (!Number.isFinite(value) || value < 0) throw new Error(`--${key.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)} must be a non-negative number.`);
    flags[key] = value;
  }
}
...
function allowedFlagsFor(command) {
  ...
  const byCommand = {
    install: ['dryRun', 'withProjectMcpConfig', 'agentsFile'], uninstall: ['dryRun', 'confirm', 'removeState', 'agentsFile', 'global', 'codexHome'],
    'install-global': ['codexHome', 'dryRun'],
    doctor: ['repair', 'dryRun'], migrate: ['dryRun'], repair: ['dryRun'], export: ['output', 'dryRun'], import: ['input', 'confirm', 'dryRun'], resume: ['runId', 'taskId', 'reason'], archive: ['runId', 'reason', 'confirm'], abandon: ['runId', 'reason', 'confirm', 'deleteFiles'], snapshot: [], status: [], 'list-runs': ['limit'], history: ['runId', 'limit'], 'memory-list': ['limit'], 'memory-inspect': ['id'], 'memory-purge-expired': ['dryRun'], 'review-project': ['maxFiles'],
    'clear-current': ['deleteFiles'], cleanup: ['minAgeMs', 'scratchMaxAgeMs', 'allSessions', 'confirm', 'dryRun'],
    'prune-history': ['retentionDays', 'dryRun'], backup: ['output', 'dryRun'], restore: ['input', 'confirm', 'dryRun'], hook: []
  };
...
async function backupStore(env, flags) {
  ...
  if (!fs.existsSync(source.path)) throw new Error(`No ${source.storage} state file exists to back up.`);
  const destination = flags.output ? path.resolve(flags.output) : path.join(source.stateDir, 'backups', `otm-${source.storage}-${Date.now()}${path.extname(source.path) || '.json'}`);
  if (flags.dryRun) return { storage: source.storage, path: destination, source: source.path, dryRun: true };
  ...
  return { storage: source.storage, path: destination, source: source.path, dryRun: false };
}

function restoreStore(env, flags) {
  if (flags.confirm !== true) throw new Error('restore requires --confirm after reviewing the backup path.');
  const source = stateFileFor(env);
  const input = path.resolve(flags.input || '');
  if (!fs.existsSync(input) || !fs.statSync(input).isFile()) throw new Error('restore input must be an existing backup file.');
  if (flags.dryRun) return { storage: source.storage, path: source.path, input, dryRun: true };
  fs.mkdirSync(path.dirname(source.path), { recursive: true });
  fs.copyFileSync(input, source.path);
  if (source.storage === 'sqlite') {
    for (const suffix of ['-wal', '-shm']) { try { fs.rmSync(`${source.path}${suffix}`, { force: true }); } catch {} }
  }
...
function stateFileFor(env) {
  ...
  return requested === 'json'
    ? { storage: 'json', stateDir, path: path.join(stateDir, 'json', 'state.json') }
    : { storage: 'sqlite', stateDir, path: path.join(stateDir, 'state.sqlite') };
}
...
function inspectMigrationDryRun(env) {
  ...
  if (requested !== 'json' && fs.existsSync(dbPath) && loadBetterSqlite3()) {
    ...
    try { return { storage: 'sqlite', currentVersion: Number(db.pragma('user_version', { simple: true }) || 0), targetVersion: SQLITE_SCHEMA_VERSION, database: dbPath }; } finally { db.close(); }
  }
  return { storage: requested === 'sqlite' ? 'sqlite-unavailable' : 'json', currentVersion: null, targetVersion: requested === 'json' ? 'otm.store.v1' : SQLITE_SCHEMA_VERSION, database: requested === 'json' ? path.join(stateDir, 'json', 'state.json') : dbPath };
}

function printCli(flags, payload, markdown) {
  console.log(flags.json === true ? JSON.stringify(payload, null, 2) : markdown);
}

function memoryMarkdown(entries) {
  return ['## OTM memory', '', ...(entries.length ? entries.map((entry) => `- ${entry.id} — ${entry.title}`) : ['No non-expired entries.']), ''].join('\n');
}
...
function packageVersion(packageRoot) {
  try { return JSON.parse(readText(path.join(packageRoot, 'package.json'), '{}')).version || '0.0.0'; } catch { return '0.0.0'; }
}
